...
VARIABILI OPZIONALI:
    ...
    BITGET_DEMO           -> "true" (default) = Demo Trading. "false" = TRADING REALE.
...
def bitget_request(method: str, path: str, params: dict = None, body: dict = None):
    ...
    if not (BITGET_API_KEY and BITGET_API_SECRET and BITGET_API_PASSPHRASE):
        raise ValueError("Credenziali Bitget (BITGET_API_KEY/SECRET/PASSPHRASE) non configurate")
...
async def esegui_apertura_trade(bot: Bot, segnale: Segnale) -> dict | None:
    ...
    logger.info(
        ...
        f"margine ~{margine_usdt:.2f} USDT (demo={BITGET_DEMO})"
    )
...
async def cmd_posizioni(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not posizioni:
        ...
        righe = [f"📋 *Posizioni reali su Bitget* {'(Demo Trading)' if BITGET_DEMO else '(LIVE)'}", "━━━━━━━━━━━━━━━"]
        for p in posizioni:
            symbol = p.get("symbol", "?")
            lato = p.get("holdSide", "?")
