...
WATCHLIST = [
    # Ridotta alle major cap su richiesta di Fabio (01/09/2026), poi ampliata (stesso
    # giorno) con altre coppie ugualmente grandi e liquide, per avere piu' opportunita'
    # senza riabbassare la qualita' del punteggio. XBTUSD = Bitcoin (naming Kraken).
    "XBTUSD", "ETHUSD", "SOLUSD", "BNBUSD",
    "XRPUSD", "ADAUSD", "DOGEUSD", "AVAXUSD", "LINKUSD", "LTCUSD",
]
...
SOGLIA_ATR_MINIMA_PERCENTO = 0.35  # sotto questa volatilita' (ATR/prezzo %), il mercato e'
# considerato troppo piatto per generare un segnale affidabile - filtro anti-rumore.
...
def analizza_coppia(pair: str, statistiche: dict = None) -> tuple[Segnale | None, int, str]:
    ...
    rsi_ok = (bias == Direzione.LONG and precedente["rsi"] < 35 and ultimo["rsi"] >= 35) or \
             (bias == Direzione.SHORT and precedente["rsi"] > 65 and ultimo["rsi"] <= 65)
    dettagli["rsi_ok"] = bool(rsi_ok)
    if rsi_ok:
        score += 25
        motivi.append("RSI in uscita da ipervenduto" if bias == Direzione.LONG else "RSI in uscita da ipercomprato")
    ...
    macd_ok = (bias == Direzione.LONG and cross_up) or (bias == Direzione.SHORT and cross_down)
    dettagli["macd_ok"] = bool(macd_ok)
    if macd_ok:
        score += 25
        motivi.append("MACD cross rialzista" if bias == Direzione.LONG else "MACD cross ribassista")
    ...
    if atr_percento < SOGLIA_ATR_MINIMA_PERCENTO:
        logger.info(f"{pair}: score {score}/100 ma volatilita' troppo bassa (ATR {atr_percento:.2f}% < {SOGLIA_ATR_MINIMA_PERCENTO}%) - mercato piatto, segnale scartato")
    ...
    if bias == Direzione.LONG:
        stop_loss = entry - (1.5 * moltiplicatore_volatilita * atr_val)
        tp1 = entry + (1.5 * moltiplicatore_volatilita * atr_val)
        tp2 = entry + (3.0 * moltiplicatore_volatilita * atr_val)
        tp3 = entry + (4.5 * moltiplicatore_volatilita * atr_val)
    else:
        stop_loss = entry + (1.5 * moltiplicatore_volatilita * atr_val)
        tp1 = entry - (1.5 * moltiplicatore_volatilita * atr_val)
        tp2 = entry - (3.0 * moltiplicatore_volatilita * atr_val)
...
async def handle_api_data(request):
    ...
    payload = {
        ...
        "monitoraggio": [MONITORAGGIO[p] for p in WATCHLIST if p in MONITORAGGIO],
    }
...
async def cmd_monitoraggio(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pair in WATCHLIST:
        ...
        macd_s = "✓" if d["macd_ok"] else "✗"
        vol_s = "✓" if d["volume_ok"] else "✗"
