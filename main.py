...
VARIABILI OPZIONALI:
    ...
    BACKUP_STATO_TELEGRAM -> "auto" (default): se non c'e' un disco permanente (es. Render
                              gratuito, niente RAILWAY_VOLUME_MOUNT_PATH), il bot salva lo stato
                              come file nella chat privata di Fabio (ADMIN_CHAT_ID, messaggio
                              fissato) e lo ricarica a ogni riavvio. "false" per spegnerlo.
    WEB_PASSWORD          -> password per aprire la dashboard anche da browser (PC, tablet),
                              fuori da Telegram, all'indirizzo https://<dominio Railway>/.
                              Almeno 12 caratteri. Se manca, l'accesso da browser e' spento
                              e la dashboard funziona solo dentro Telegram come prima.
...
from telegram import Bot, InlineKeyboardButton, InlineKeyboardMarkup, InputMediaDocument, WebAppInfo
...
# In LIVE (soldi reali) l'analisi e' piu' approfondita (piu' ricerche web, piu' fonti
# incrociate: asset + settore/categoria + eventi macro) - richiede piu' tempo del
# controllo leggero usato in Demo Trading. Aggiunto 05/09/2026 su richiesta di Fabio:
# "che studi approfonditamente il mercato in questione e le news prima di puntare al
# profitto". NOTA IMPORTANTE PER FABIO: questo resta un filtro di RISCHIO in piu', non
# una bacchetta magica - nessuna analisi, per quanto approfondita, garantisce un trade
# vincente. Puo' solo abbassare la probabilita' di entrare in un momento chiaramente
# sbagliato (notizie fresche, eventi macro imminenti), riducendo la size se il quadro
# e' incerto invece di aprire/non aprire in modo netto.
TIMEOUT_IA_SECONDI_LIVE = 100
MOLTIPLICATORE_SIZE_MINIMO_IA = 0.4  # mai ridurre la size sotto il 40% di quella prevista dal rischio configurato
# Categorie per cui ha senso verificare il prezzo Bitget contro una fonte di mercato
# indipendente (05/09/2026, su richiesta di Fabio: "non deve guardare solo Bitget, deve
# scavare a fondo su siti attendibili e direttamente in borsa"). Le azioni/ETF/materie
# prime su Bitget sono strumenti TOKENIZZATI che replicano un prezzo reale esterno - ha
# senso controllare che lo facciano bene. Le crypto major sono escluse apposta: li' il
# perpetuo Bitget stesso E' uno dei mercati di riferimento, non una replica di qualcos'altro,
# quindi un confronto "vs fonte esterna" non avrebbe un termine di paragone sensato.
CATEGORIE_VERIFICA_PREZZO_ESTERNO = {"azioni", "etf", "materie_prime"}
SOGLIA_DIVERGENZA_PREZZO_PERCENTO = 1.0     # oltre questa % di scostamento: size ridotta
SOGLIA_DIVERGENZA_PREZZO_VETO_PERCENTO = 3.0  # oltre questa %: trade annullato del tutto
...
LUNGHEZZA_MINIMA_WEB_PASSWORD = 12
WEB_PORT = int(os.environ.get("PORT", "8080"))
RAILWAY_PUBLIC_DOMAIN = os.environ.get("RAILWAY_PUBLIC_DOMAIN", "")
# Su Render l'indirizzo pubblico arriva in RENDER_EXTERNAL_HOSTNAME / RENDER_EXTERNAL_URL.
RENDER_EXTERNAL_URL = os.environ.get("RENDER_EXTERNAL_URL", "").rstrip("/")
DOMINIO_PUBBLICO = RAILWAY_PUBLIC_DOMAIN or os.environ.get("RENDER_EXTERNAL_HOSTNAME", "")
...
CONFIG = {
    ...
    # Categorie di mercati su cui operare - modificabili dalla dashboard.
    # Default aggiornato 05/09/2026 (su richiesta di Fabio): TUTTE le categorie attive
    # (non solo azioni/materie prime), ciascuna pero' con soglie atr_min/ema_min proprie
    # e non piu' i valori permissivi di prima (vedi SOGLIE_PER_CATEGORIA_DEFAULT sotto).
    # Resta un default di PARTENZA facoltativo - disattiva quello che vuoi dalla dashboard.
    "categorie_attive": [
        "crypto_major", "azioni", "etf", "materie_prime", "ai", "meme",
        "solana_eco", "gamefi_nft", "defi", "pagamenti", "depin", "stablecoin",
    ],
    # --- Parametri di selezione segnale, tutti modificabili dalla dashboard senza
    # toccare il codice (aggiunto 05/09/2026, su richiesta di Fabio dopo l'analisi
    # del win rate reale). I valori sotto sono gia' TARATI per azioni/materie prime
    # (non per crypto) - vedi il ragionamento nel commento sopra SOGLIE_PER_CATEGORIA_DEFAULT.
    # Restano un punto di partenza facoltativo: tutto modificabile dalla dashboard in
    # qualsiasi momento, niente e' bloccato nel codice.
    #
    # Motivo dei valori: azioni e materie prime si muovono con meno "rumore" wick-to-wick
    # delle crypto (niente scambi 24/7 dominati da retail/leva estrema), e tendono a
    # sviluppare trend piu' puliti una volta partiti. Questo permette:
    #  - una soglia di ingresso leggermente PIU' ALTA (70 invece di 65): qui non serve
    #    "riempire" il canale con molti segnali al giorno come nelle crypto, meglio
    #    pochi setup piu' selettivi;
    #  - un TP1 relativamente piu' vicino allo SL (1.5x contro 1.8x, rapporto 0.83 - quasi
    #    pari, non piu' 0.6 come nelle crypto): con meno rumore, il prezzo che si muove a
    #    favore prima di toccare lo SL e' un segnale piu' affidabile che il trend sia vero;
    #  - finestre di conferma RSI/MACD piu' strette (3 candele/2 ore invece di 5/3): i
    #    mercati regolamentati aprono/chiudono a orari fissi e reagiscono piu' rapidamente
    #    alle notizie, un segnale "vecchio" di 5 ore perde rilevanza piu' in fretta che
    #    in un mercato crypto sempre aperto.
    "score_minimo": 70,                 # soglia di punteggio (su 100) per pubblicare/aprire un segnale
    "finestra_rsi_candele": 3,          # quante candele indietro cerchiamo l'ipervenduto/ipercomprato RSI
    "finestra_macd_candele": 2,         # quante ore indietro accettiamo un incrocio MACD come "recente"
    "atr_moltiplicatore_sl": 1.8,       # distanza dello Stop Loss, in multipli di ATR
    "atr_moltiplicatore_tp1": 1.5,      # distanza del primo Take Profit (1/3 posizione, poi SL a breakeven)
    "atr_moltiplicatore_tp2": 3.0,      # distanza del secondo Take Profit (1/3 posizione, poi SL a TP1)
    "atr_moltiplicatore_tp3": 5.0,      # distanza del terzo Take Profit (chiusura finale)
    # Soglie di filtro per categoria (volatilita' minima e distanza minima EMA50/200
    # perche' un trend sia considerato affidabile). Struttura: {chiave_categoria:
    # {"atr_min": float, "ema_min": float}}. Solo le categorie qui presenti
    # sovrascrivono il default di fabbrica in SOGLIE_PER_CATEGORIA_DEFAULT; le altre
    # usano il default finche' Fabio non le modifica dalla dashboard. Lasciato vuoto
    # apposta: i default gia' calibrati per azioni/materie_prime stanno direttamente
    # in SOGLIE_PER_CATEGORIA_DEFAULT (vedi sotto) - qui andranno solo le eventuali
    # correzioni fatte in seguito dalla dashboard.
    "soglie_categorie": {},
    # Rispetta gli orari di apertura di Wall Street per la categoria "azioni" (05/09/2026,
    # su richiesta di Fabio: le azioni tokenizzate replicano il prezzo reale della borsa
    # USA, quindi fuori orario il prezzo e' fermo/poco liquido - aprire li' un trade
    # rischia entry pessime). Facoltativo: disattivabile se in futuro Bitget offrisse
    # scambi fuori orario realmente liquidi su questi strumenti.
    "rispetta_orari_azioni": True,
    # Ogni quanti secondi il bot riscansiona tutti i mercati attivi in cerca di nuovi
    # segnali, e ricontrolla lo stato delle posizioni aperte (05/09/2026, su richiesta
    # di Fabio, modificabile dalla dashboard). Il valore di fabbrica (600s = 10 minuti)
    # e' un buon compromesso fra reattivita' e numero di chiamate API: un valore piu'
    # basso reagisce piu' in fretta sia a nuovi segnali sia a SL/TP sulle posizioni
    # aperte, ma aumenta il carico sull'API di Bitget.
    "intervallo_scansione_secondi": 600,
    # Studio CONTINUO del contesto generale di mercato per categoria, da fonti esterne
    # a Bitget (05/09/2026, su richiesta esplicita di Fabio). Vedi
    # aggiorna_contesto_mercato_categoria() per i dettagli. Facoltativo: disattivabile
    # dalla dashboard se si preferisce risparmiare sulle chiamate API.
    "studio_mercato_globale_attivo": True,
    "studio_mercato_intervallo_ore": 3,
    # Se True, ogni perdita consecutiva alza temporaneamente la soglia score richiesta
    # (protezione anti "revenge trading" - non abbassare la selettivita' proprio subito
    # dopo una perdita). Disattivato di default (05/09/2026) su scelta esplicita di
    # Fabio dopo aver visto la soglia salire da 80 a 85 dopo un solo Stop Loss.
    "escalation_soglia_dopo_perdite_attiva": False,
    # Quante posizioni il bot puo' tenere aperte insieme (20/09/2026). Con capitale
    # piccolo conviene UNA sola: tutto il rischio disponibile finisce in un unico trade,
    # che risulta abbastanza grande da poter essere diviso in 3 livelli di Take Profit
    # sopra il minimo d'ordine di Bitget (5 USDT). Alzandolo si diversifica di piu', ma
    # ogni posizione diventa piu' piccola e i TP parziali possono ridursi a 2 o a 1.
    "max_posizioni_aperte": 1,
}
...
CATEGORIE_WATCHLIST = {
    ...
    "etf": {
        "nome": "ETF",
        "simboli": ["SPYUSD", "QQQUSD", "DIAUSD", "IWMUSD"],
    },
    "materie_prime": {
    ...
    # Stablecoin (05/09/2026, su richiesta di Fabio) - NON passano per il sistema di
    # score trend-following (EMA/RSI/MACD non hanno senso su un asset che per
    # definizione non deve muoversi). Vengono invece monitorate per un DEPEG - vedi
    # controlla_depeg_stablecoin() - e generano solo un'allerta Telegram, MAI
    # un'apertura automatica di posizione: un depeg reale (es. UST nel 2022) puo'
    # essere permanente e nessuno Stop Loss a leva salva in tempo.
    "stablecoin": {
        "nome": "Stablecoin (monitoraggio depeg)",
        "simboli": ["USDCUSD", "FDUSDUSD", "DAIUSD", "TUSDUSD", "USDEUSD"],
    },
}
...
SCAN_INTERVAL_SECONDS = 600       # valore di fabbrica/fallback - quello davvero usato e' CONFIG["intervallo_scansione_secondi"], modificabile dalla dashboard
# Valore di fabbrica - quello davvero usato e' CONFIG["score_minimo"], modificabile
# dalla dashboard (vedi soglia_score_dinamica). Questa costante resta solo come
# fallback e riferimento nei messaggi statici del bot.
SCORE_MINIMO_PUBBLICAZIONE_DEFAULT = 65   # su 100 - confermato dal backtest: EV +0.043R/trade, ~15 segnali/giorno
...
class Segnale:
    ...
    # Quale strategia (Expert Advisor) ha generato il segnale - serve per le statistiche
    # separate per strategia. Default "completo" cosi' le posizioni salvate su disco da
    # versioni precedenti (senza questo campo) si ricaricano senza errori.
    strategia: str = "completo"
...
# ---------------------------------------------------------------------------
# BACKUP DELLO STATO SU TELEGRAM (per hosting senza disco permanente, es. Render gratis)
# ---------------------------------------------------------------------------
# Il file di stato viene tenuto come documento in UN solo messaggio fissato nella chat
# privata di Fabio con il bot: a ogni aggiornamento il documento del messaggio viene
# sostituito (niente spam di messaggi, nessuna notifica). All'avvio, se sul disco non
# c'e' stato, il bot scarica quel documento e riparte da li'.

NOME_FILE_BACKUP = "stato_bot_backup.json"
INTERVALLO_MINIMO_BACKUP_SECONDI = 300
_BACKUP = {"message_id": None, "hash": None, "ultimo": 0.0}


def backup_telegram_attivo() -> bool:
    scelta = os.environ.get("BACKUP_STATO_TELEGRAM", "auto").strip().lower()
    if scelta in ("false", "0", "no", "off") or not ADMIN_CHAT_ID:
        return False
    if scelta in ("true", "1", "si", "yes", "on"):
        return True
    return not os.environ.get("RAILWAY_VOLUME_MOUNT_PATH")  # auto: solo se manca un disco permanente


async def ripristina_stato_da_telegram(bot: Bot):
    if not backup_telegram_attivo():
        return
    if os.path.exists(STATO_FILE):
        logger.info("Stato gia' presente su disco: non serve ripristinarlo da Telegram.")
        return
    try:
        chat = await bot.get_chat(ADMIN_CHAT_ID)
        msg = chat.pinned_message
        documento = getattr(msg, "document", None) if msg else None
        if not documento or documento.file_name != NOME_FILE_BACKUP:
            logger.info("Nessun backup dello stato fissato nella chat admin: parto da zero.")
            return
        file = await bot.get_file(documento.file_id)
        contenuto = bytes(await file.download_as_bytearray())
        json.loads(contenuto)  # verifica che sia un JSON valido prima di usarlo
        os.makedirs(os.path.dirname(os.path.abspath(STATO_FILE)), exist_ok=True)
        with open(STATO_FILE, "wb") as f:
            f.write(contenuto)
        _BACKUP["message_id"] = msg.message_id
        _BACKUP["hash"] = hashlib.sha256(contenuto).hexdigest()
        logger.info(f"Stato ripristinato dal backup su Telegram ({len(contenuto)} byte).")
    except Exception as e:
        logger.warning(f"Ripristino dello stato da Telegram non riuscito ({e}): parto da zero.")


async def backup_stato_su_telegram(bot: Bot, forza: bool = False):
    if not backup_telegram_attivo() or not os.path.exists(STATO_FILE):
        return
    try:
        with open(STATO_FILE, "rb") as f:
            contenuto = f.read()
        impronta = hashlib.sha256(contenuto).hexdigest()
        if impronta == _BACKUP["hash"]:
            return
        if not forza and time.time() - _BACKUP["ultimo"] < INTERVALLO_MINIMO_BACKUP_SECONDI:
            return
        didascalia = ("💾 Backup automatico del bot (statistiche, posizioni seguite, impostazioni). "
                      "Non cancellarlo e non toglierlo dai fissati. Aggiornato: "
                      + datetime.now(ZoneInfo("Europe/Rome")).strftime("%d/%m %H:%M"))
        inviato = False
        if _BACKUP["message_id"]:
            try:
                await bot.edit_message_media(
                    chat_id=ADMIN_CHAT_ID, message_id=_BACKUP["message_id"],
                    media=InputMediaDocument(media=io.BytesIO(contenuto), filename=NOME_FILE_BACKUP, caption=didascalia),
                )
                inviato = True
            except Exception as e:
                logger.warning(f"Aggiornamento del messaggio di backup non riuscito ({e}), ne creo uno nuovo.")
        if not inviato:
            msg = await bot.send_document(
                chat_id=ADMIN_CHAT_ID, document=io.BytesIO(contenuto), filename=NOME_FILE_BACKUP,
                caption=didascalia, disable_notification=True,
            )
            _BACKUP["message_id"] = msg.message_id
            try:
                await bot.pin_chat_message(chat_id=ADMIN_CHAT_ID, message_id=msg.message_id, disable_notification=True)
            except Exception as e:
                logger.warning(f"Impossibile fissare il messaggio di backup: {e}")
        _BACKUP["hash"] = impronta
        _BACKUP["ultimo"] = time.time()
    except Exception as e:
        logger.warning(f"Backup dello stato su Telegram non riuscito: {e}")
...
def carica_stato():
    ...
    try:
        ...
        # BUG corretto 05/09/2026: questi due campi venivano salvati (salva_stato scrive
        # tutta la CONFIG) ma MAI riletti qui - ogni redeploy su Railway faceva tornare
        # categorie_attive e modalita_segnale ai default di fabbrica (crypto_major,
        # "completo"), anche se Fabio li aveva cambiati dalla dashboard.
        CONFIG["modalita_segnale"] = config_salvato.get("modalita_segnale", CONFIG["modalita_segnale"])
        CONFIG["categorie_attive"] = config_salvato.get("categorie_attive", CONFIG["categorie_attive"])
        # Nuovi parametri di selezione segnale (05/09/2026) - fallback ai default in
        # CONFIG se lo stato su disco e' di una versione precedente e non li contiene.
        CONFIG["score_minimo"] = config_salvato.get("score_minimo", CONFIG["score_minimo"])
        CONFIG["finestra_rsi_candele"] = config_salvato.get("finestra_rsi_candele", CONFIG["finestra_rsi_candele"])
        CONFIG["finestra_macd_candele"] = config_salvato.get("finestra_macd_candele", CONFIG["finestra_macd_candele"])
        CONFIG["atr_moltiplicatore_sl"] = config_salvato.get("atr_moltiplicatore_sl", CONFIG["atr_moltiplicatore_sl"])
        CONFIG["atr_moltiplicatore_tp1"] = config_salvato.get("atr_moltiplicatore_tp1", CONFIG["atr_moltiplicatore_tp1"])
        CONFIG["atr_moltiplicatore_tp2"] = config_salvato.get("atr_moltiplicatore_tp2", CONFIG["atr_moltiplicatore_tp2"])
        CONFIG["atr_moltiplicatore_tp3"] = config_salvato.get("atr_moltiplicatore_tp3", CONFIG["atr_moltiplicatore_tp3"])
        CONFIG["soglie_categorie"] = config_salvato.get("soglie_categorie", CONFIG["soglie_categorie"])
        CONFIG["rispetta_orari_azioni"] = config_salvato.get("rispetta_orari_azioni", CONFIG["rispetta_orari_azioni"])
        CONFIG["intervallo_scansione_secondi"] = config_salvato.get("intervallo_scansione_secondi", CONFIG["intervallo_scansione_secondi"])
        CONFIG["studio_mercato_globale_attivo"] = config_salvato.get("studio_mercato_globale_attivo", CONFIG["studio_mercato_globale_attivo"])
        CONFIG["studio_mercato_intervallo_ore"] = config_salvato.get("studio_mercato_intervallo_ore", CONFIG["studio_mercato_intervallo_ore"])
        CONFIG["escalation_soglia_dopo_perdite_attiva"] = config_salvato.get("escalation_soglia_dopo_perdite_attiva", CONFIG["escalation_soglia_dopo_perdite_attiva"])
...
def pulisci_ohlc(df: pd.DataFrame, pair: str = "") -> pd.DataFrame:
    """Rimuove candele chiaramente corrotte prima che finiscano negli indicatori (05/09/2026,
    su richiesta di Fabio: "deve andare su dati puliti"). Un singolo prezzo errato puo'
    falsare EMA/RSI/ATR per decine di candele successive, sia nel bot live sia nel backtest,
    quindi la pulizia va fatta qui, in un unico punto usato da entrambi.

    Cosa rimuove:
      1. Righe con valori mancanti, prezzi zero/negativi, o candele impossibili (high < low)
      2. Timestamp duplicati (tiene l'ultimo ricevuto)
      3. Outlier statistici: candele il cui prezzo di chiusura si discosta dalla mediana
         mobile locale per piu' di SOGLIA_OUTLIER_DEVIAZIONI deviazioni standard - un
         filtro volutamente conservativo (una vera notizia shock puo' comunque produrre
         un movimento cosi' ampio, ma e' un compromesso migliore che lasciar passare un
         tick chiaramente rotto)

    LIMITE ONESTO: non conosce eventi corporate reali (split azionari, dividendi
    straordinari) - se Bitget non aggiusta da solo il prezzo storico di un'azione
    tokenizzata dopo uno split, questo filtro potrebbe scambiare un salto legittimo
    per un outlier, o viceversa non riconoscere come anomalo un dato che in realta'
    lo e' per quel motivo. Resta un filtro statistico, non una verifica sull'evento
    reale sottostante."""
    if df.empty:
        return df
    ...
    # 1) Righe strutturalmente invalide
    df = df.dropna(subset=["open", "high", "low", "close", "volume"])
    df = df[(df["open"] > 0) & (df["high"] > 0) & (df["low"] > 0) & (df["close"] > 0)]
    df = df[df["high"] >= df["low"]]

    # 2) Duplicati sul timestamp (tiene l'ultimo, spesso il piu' aggiornato/corretto)
    df = df.drop_duplicates(subset=["time"], keep="last")
    df = df.sort_values("time").reset_index(drop=True)

    # 3) Outlier statistici sul prezzo di chiusura, con mediana e deviazione mobile.
    # IMPORTANTE (corretto 05/09/2026 su segnalazione di Fabio): la finestra e'
    # "trailing" (solo candele PASSATE fino a quella corrente), MAI centrata. Una
    # finestra centrata guarderebbe anche candele future rispetto a quella valutata -
    # cosa impossibile in tempo reale (il bot non puo' usare dati che non esistono
    # ancora) e comunque scorretta anche nel backtest, dove userebbe implicitamente
    # informazioni dal futuro per giudicare il passato (lookahead bias: il backtest
    # sembrerebbe piu' pulito/affidabile di quanto sarebbe stato in tempo reale).
    # Uso MAD (Median Absolute Deviation) invece della deviazione standard classica:
    # la std normale viene "inquinata" dallo stesso outlier che dovrebbe rilevare (un
    # solo prezzo corrotto in una finestra di 20 gonfia la std a tal punto che lo
    # spike stesso non supera piu' la soglia) - MAD e' robusta a questo perche' usa
    # la mediana anche per misurare la dispersione, non solo il centro.
    if len(df) >= 30:
        def _mad_locale(x):
            mediana = np.median(x)
            return np.median(np.abs(x - mediana))

        # shift(1): la soglia di riferimento per la candela i usa SOLO le 20 candele
        # precedenti, mai la candela i stessa - altrimenti un prezzo corrotto
        # contribuirebbe (anche se di poco) alla propria stessa soglia di giudizio.
        mediana_mobile = df["close"].rolling(20, min_periods=15).median().shift(1)
        mad_mobile = df["close"].rolling(20, min_periods=15).apply(_mad_locale, raw=True).shift(1)
        std_robusto = mad_mobile * 1.4826  # rende la MAD comparabile a una deviazione standard su dati normali
        z_score = (df["close"] - mediana_mobile).abs() / std_robusto.replace(0, np.nan)
        maschera_outlier = z_score > SOGLIA_OUTLIER_DEVIAZIONI
        n_outlier = int(maschera_outlier.fillna(False).sum())
        if n_outlier > 0:
            logger.info(f"{pair}: rimosse {n_outlier} candele anomale (outlier statistico) su {len(df)}")
            df = df[~maschera_outlier.fillna(False)]

    df = df.reset_index(drop=True)
    n_rimosse = n_iniziale - len(df)
    if n_rimosse > 0:
        logger.info(f"{pair}: pulizia dati OHLC - rimosse {n_rimosse}/{n_iniziale} candele totali")
    return df


SOGLIA_OUTLIER_DEVIAZIONI = 6.0  # quante deviazioni standard dalla mediana mobile per considerare una candela anomala
...
def get_ohlc(pair: str, interval: int, count: int = 250) -> pd.DataFrame:
    ...
    df = df.sort_values("time").reset_index(drop=True)
    df = pulisci_ohlc(df, pair)
    if df.empty:
        raise ValueError(f"Tutte le candele per {simbolo} sono state scartate dalla pulizia dati")
...
def get_chiusura_reale(pair: str, aperta_dal: datetime = None) -> dict | None:
    """Come si e' chiusa DAVVERO l'ultima posizione su questa coppia, secondo Bitget:
    prezzo medio di chiusura e profitto netto (commissioni incluse). Serve perche' una
    posizione puo' sparire per motivi che il bot non controlla - chiusura manuale
    dall'app, chiusura parziale a mano, liquidazione - e in quei casi dedurre l'esito
    dagli ordini piano porterebbe a segnare come "Stop Loss" un trade magari chiuso in
    profitto (successo in produzione il 20/09/2026). Ritorna None se l'informazione non
    e' disponibile: in quel caso si torna alla vecchia deduzione dai TP raggiunti."""
    symbol = pair_kraken_a_bitget(pair)
    params = {"symbol": symbol, "productType": "USDT-FUTURES", "limit": "20"}
    if aperta_dal:
        # Un minuto di margine: l'orario del bot e quello di Bitget possono non coincidere.
        params["startTime"] = str(int((aperta_dal - timedelta(minutes=1)).timestamp() * 1000))
    try:
        dati = bitget_request("GET", "/api/v2/mix/position/history-position", params=params)
    except Exception as e:
        logger.warning(f"{pair}: impossibile leggere la chiusura reale da Bitget ({e}), uso la deduzione dai TP.")
        return None

    lista = dati.get("list") if isinstance(dati, dict) else dati
    if not lista:
        return None
    for riga in lista:  # la piu' recente e' in cima
        if riga.get("symbol") != symbol:
            continue
        try:
            return {
                "prezzo_chiusura": float(riga.get("closeAvgPrice") or 0) or None,
                "pnl": float(riga.get("netProfit")),
            }
        except (TypeError, ValueError):
            return None
    return None
...
# --- Contesto specifico per categoria, usato nell'analisi approfondita LIVE (05/09/2026).
# Ogni tipo di mercato reagisce a driver diversi: un'azione si muove per utili/tassi, una
# materia prima per domanda/offerta e geopolitica, una crypto per regolamentazione e flussi
# on-chain. Dare all'IA il contesto giusto invece di un prompt generico "cerca notizie"
# produce una ricerca piu' mirata.
CONTESTO_RICERCA_PER_CATEGORIA = {
    "azioni": "utili societari recenti o attesi, guidance, tassi d'interesse Fed/BCE, "
              "notizie specifiche sull'azienda (cause legali, cambio management, M&A), "
              "andamento dell'indice/settore di riferimento",
    "materie_prime": "dati di domanda/offerta (scorte, produzione OPEC per il petrolio, "
                      "raccolti per le agricole), eventi geopolitici che influenzano le "
                      "forniture, dati macro su dollaro USA (le materie prime sono quotate in USD)",
    "crypto_major": "notizie regolatorie (SEC, ETF, divieti), movimenti di whale/exchange "
                     "noti, correlazione con BTC come mercato guida, eventi di rete (upgrade, exploit)",
    "ai": "notizie regolatorie o di settore tech, annunci di prodotto/partnership legati al progetto",
    "etf": "flussi di capitale in entrata/uscita dal fondo, composizione e rotazione settoriale "
           "dei titoli sottostanti, dati macro USA (tassi, inflazione, occupazione)",
    "meme": "sentiment sui social (X/Twitter, Reddit), eventuali endorsement o pump coordinati",
    "solana_eco": "salute della rete Solana, eventi di rete, notizie sull'ecosistema DeFi collegato",
    "gamefi_nft": "volume di trading NFT/gaming, annunci di gioco o partnership",
    "defi": "TVL (total value locked) del protocollo, notizie di sicurezza/exploit nel settore DeFi",
    "pagamenti": "adozione reale del progetto, partnership con istituzioni finanziarie",
    "depin": "adozione della rete fisica sottostante, notizie di settore infrastrutturale",
}


# --- Studio CONTINUO del contesto generale di mercato, per categoria (05/09/2026, su
# richiesta esplicita di Fabio: "non deve guardare solo Bitget, deve studiare
# approfondito ogni momento... siti degli Stati Uniti o siti attendibili"). ---
#
# DIFFERENZA rispetto ad analisi_approfondita_live: quella analizza UN singolo asset
# SOLO quando un segnale tecnico e' gia' pronto per essere aperto. Questa invece studia
# lo stato GENERALE del mercato (es. "Wall Street oggi e' in un momento di forte
# incertezza per via del dato sull'inflazione delle 14:30") ad intervalli regolari,
# INDIPENDENTEMENTE dal fatto che ci sia o meno un segnale pronto - e usa quello che
# trova per rendere il bot piu' o meno selettivo su un'intera categoria per le prossime
# ore, non solo sul singolo trade.
#
# PERCHE' non ad ogni ciclo di scansione (ogni pochi minuti) per ogni singolo asset:
# interrogare l'IA con ricerca web per ognuno dei 60+ mercati ogni 10 minuti sarebbe
# enormemente costoso e lento senza aggiungere valore reale - le condizioni generali di
# un mercato (regime di rischio, eventi macro del giorno) cambiano su scala di ore, non
# di minuti. Il controllo per-asset piu' fitto (analisi_approfondita_live) resta al
# momento dell'apertura, quando conta davvero.
CATEGORIE_STUDIO_GLOBALE = {"azioni", "etf", "materie_prime", "crypto_major"}
_CONTESTO_MERCATO_CATEGORIA: dict = {}  # categoria -> {"timestamp":, "bonus_soglia":, "rischio_alto":, "motivo":}


async def aggiorna_contesto_mercato_categoria(bot: Bot, categoria: str) -> dict:
    """Aggiorna (se scaduta la cache) lo studio generale di mercato per una categoria,
    interrogando l'IA con ricerca web su fonti finanziarie affidabili (prioritariamente
    americane per azioni/ETF, dato che Wall Street e' il mercato di riferimento).
    Ritorna sempre un dizionario utilizzabile, anche in caso di errore (bonus_soglia=0,
    nessun impatto) - questo studio raffina la selettivita', non blocca il bot se non
    disponibile."""
    default = {"timestamp": None, "bonus_soglia": 0, "rischio_alto": False, "motivo": "non ancora studiato"}
    if not CONFIG.get("studio_mercato_globale_attivo", True) or not ANTHROPIC_API_KEY:
        return default

    cache = _CONTESTO_MERCATO_CATEGORIA.get(categoria)
    intervallo = timedelta(hours=CONFIG.get("studio_mercato_intervallo_ore", 3))
    if cache and (datetime.now() - cache["timestamp"]) < intervallo:
        return cache  # ancora fresco, nessuna nuova chiamata IA

    focus_categoria = CONTESTO_RICERCA_PER_CATEGORIA.get(categoria, "notizie generali di mercato")
    fonte_suggerita = (
        "Priorita' a fonti statunitensi/istituzionali (SEC, Federal Reserve, Bureau of "
        "Labor Statistics, Wall Street Journal, Bloomberg, Reuters, CNBC) dato che questi "
        "strumenti replicano il mercato USA."
        if categoria in ("azioni", "etf") else
        "Fonti finanziarie affidabili e riconosciute, non blog generici o forum."
    )

    prompt = (
        f"Sei un analista macro. Devi valutare lo stato GENERALE ORA del mercato "
        f"'{categoria}' (non un singolo asset specifico), per decidere quanto un bot di "
        f"trading automatico debba essere prudente nelle prossime ore.\n\n"
        f"Cerca sul web, con piu' query se serve, informazioni aggiornate su: {focus_categoria}. "
        f"{fonte_suggerita}\n"
        f"Controlla anche se oggi cadono eventi macro ad alto impatto (riunioni banche "
        f"centrali, dati su inflazione/occupazione USA, festivita' di borsa) che renderebbero "
        f"prudente alzare la soglia di ingresso per le prossime ore.\n\n"
        f"Rispondi SOLO con un JSON, senza altro testo prima o dopo:\n"
        f'{{"rischio_alto": true/false, "bonus_soglia": 0-20, "motivo": "1-2 frasi in italiano '
        f'che citino cosa hai trovato"}}\n\n'
        f'- "rischio_alto": true se c\'e\' un evento importante oggi o una condizione di '
        f"mercato chiaramente anomala (volatilita' estrema, notizia shock di settore)\n"
        f'- "bonus_soglia": quanti punti aggiungere alla soglia di score richiesta per '
        f"aprire un trade in questa categoria nelle prossime ore (0 = nessun motivo di "
        f"cautela extra, 10-20 = giornata da trattare con molta prudenza)"
    )
    ...
    def _chiama_api():
        risposta = requests.post(
            ...
            timeout=TIMEOUT_IA_SECONDI_LIVE,
        )
        risposta.raise_for_status()
        return risposta.json()
    ...
    try:
        ...
        inizio, fine = testo_pulito.find("{"), testo_pulito.rfind("}")
        if inizio == -1 or fine == -1:
            logger.warning(f"Studio mercato '{categoria}': risposta IA senza JSON valido: {testo[:300]}")
            risultato = default
        else:
            verdetto = json.loads(testo_pulito[inizio:fine + 1])
            try:
                bonus = int(verdetto.get("bonus_soglia", 0))
            except (TypeError, ValueError):
                bonus = 0
            ...
            risultato = {
                ...
                "bonus_soglia": bonus,
                "rischio_alto": bool(verdetto.get("rischio_alto", False)),
                "motivo": str(verdetto.get("motivo", ""))[:400],
            }
        logger.info(f"Studio mercato '{categoria}' aggiornato: {risultato}")

        # Notifica Fabio solo quando qualcosa di rilevante cambia (rischio_alto appena
        # scattato) - non ad ogni refresh di routine, per non spammare il canale.
        era_a_rischio = cache.get("rischio_alto") if cache else False
        if risultato.get("rischio_alto") and not era_a_rischio:
            try:
                await notifica_admin(
                    bot,
                    f"🌐 *Studio di mercato — {categoria}*\n"
                    f"Rilevata condizione di maggiore cautela: soglia di ingresso alzata di "
                    f"+{risultato['bonus_soglia']} punti per le prossime "
                    f"{CONFIG.get('studio_mercato_intervallo_ore', 3)}h.\n_{risultato['motivo']}_"
                )
            except Exception:
                pass

        _CONTESTO_MERCATO_CATEGORIA[categoria] = risultato if risultato.get("timestamp") else default
        return _CONTESTO_MERCATO_CATEGORIA[categoria]
    except Exception as e:
        logger.warning(f"Studio mercato '{categoria}' fallito ({e}) - nessun impatto sulla soglia.")
        return cache if cache else default


async def analisi_approfondita_live(segnale: Segnale) -> tuple[bool, float, str]:
    """Versione approfondita del controllo IA, usata SOLO quando il bot opera in LIVE
    (soldi reali) - vedi TIMEOUT_IA_SECONDI_LIVE. Rispetto al controllo leggero usato in
    Demo Trading (parere_ia_su_segnale):
      - piu' ricerche web permesse (fino a 10 invece di 4), su siti affidabili (fonti
        finanziarie riconosciute), non solo generiche
      - contesto specifico per categoria di mercato (azioni/materie prime/crypto hanno
        driver di prezzo molto diversi, vedi CONTESTO_RICERCA_PER_CATEGORIA)
      - per azioni/ETF/materie prime (CATEGORIE_VERIFICA_PREZZO_ESTERNO): verifica il
        prezzo Bitget contro una fonte di mercato indipendente (Bitget offre questi
        strumenti come TOKEN che replicano il prezzo reale di borsa - un disallineamento
        segnala un problema di tracking, non solo di analisi)
      - non e' un semplice si'/no: puo' ridurre la size del trade (moltiplicatore tra
        MOLTIPLICATORE_SIZE_MINIMO_IA e 1.0) quando il quadro e' incerto ma non
        abbastanza negativo da giustificare un veto secco

    Ritorna (procedere, moltiplicatore_size, motivazione). In caso di chiave mancante,
    errore o timeout: procede comunque con moltiplicatore 1.0 - stesso principio del
    controllo leggero, e' un filtro in piu' non un blocco totale se il servizio non
    risponde (non lasciare Fabio senza poter fare trading per un problema di rete)."""
    if not ANTHROPIC_API_KEY:
        return True, 1.0, "controllo IA non configurato - trade LIVE eseguito senza analisi approfondita"

    categoria = categoria_di(segnale.coppia)
    contesto_categoria = CONTESTO_RICERCA_PER_CATEGORIA.get(
        categoria,
        "notizie recenti rilevanti su questo asset e sul suo settore",
    )
    richiede_verifica_prezzo = categoria in CATEGORIE_VERIFICA_PREZZO_ESTERNO

    istruzione_prezzo = ""
    if richiede_verifica_prezzo:
        istruzione_prezzo = (
            f"\n4) IMPORTANTE - verifica il prezzo: {segnale.coppia} su Bitget e' uno strumento "
            f"tokenizzato che dovrebbe replicare il prezzo reale di borsa. Cerca su una fonte "
            f"finanziaria affidabile (Google Finance, Yahoo Finance, Investing.com, il sito "
            f"della borsa stessa, o un'agenzia come Reuters/Bloomberg) il prezzo di mercato "
            f"REALE e attuale di questo strumento, e riportalo nel campo \"prezzo_verificato\" "
            f"del JSON. Se non riesci a trovarlo con ragionevole certezza, metti null - non "
            f"inventare un numero.\n"
        )

    prompt = (
        f"Sei un analista di rischio senior per un fondo che sta per aprire una posizione "
        f"REALE (non demo, soldi veri) con questi parametri, gia' validati dagli indicatori "
        f"tecnici del sistema:\n\n"
        f"- Strumento: {segnale.coppia} (categoria: {categoria})\n"
        f"- Direzione: {segnale.direzione.value}\n"
        f"- Prezzo di ingresso (da Bitget): {segnale.entry}\n"
        f"- Stop Loss: {segnale.stop_loss}\n"
        f"- Take Profit 1/2/3: {segnale.tp1} / {segnale.tp2} / {segnale.take_profit}\n"
        f"- Analisi tecnica che ha generato il segnale: {segnale.motivazione}\n\n"
        f"Fai una ricerca approfondita, usando piu' query mirate se serve, su fonti "
        f"finanziarie riconosciute (non blog generici o forum) per:\n"
        f"1) Notizie delle ultime 24-48 ore specifiche su {segnale.coppia}\n"
        f"2) {contesto_categoria}\n"
        f"3) Eventi macro imminenti (prossime ore/giorni) che potrebbero muovere questo "
        f"mercato: dati economici, decisioni di banche centrali, festivita' che riducono "
        f"la liquidita', earnings season se rilevante"
        f"{istruzione_prezzo}\n"
        f"Poi rispondi SOLO con un JSON, senza altro testo prima o dopo:\n"
        f'{{"procedere": true/false, "moltiplicatore_size": 0.0-1.0, '
        f'"prezzo_verificato": numero o null, "motivo": "spiegazione breve in italiano, '
        f'2-3 frasi, che citi cosa hai trovato e le fonti"}}\n\n'
        f"Linee guida per il verdetto:\n"
        f'- "procedere": false SOLO se trovi qualcosa di concreto e recente che rende questo '
        f"ingresso chiaramente pericoloso ORA (notizia molto negativa appena uscita, evento "
        f"macro ad altissimo impatto nelle prossime ore, mercato in un movimento estremo "
        f"appena avvenuto nella direzione opposta al trade, oppure un forte disallineamento "
        f"tra il prezzo Bitget e quello reale di borsa)\n"
        f'- "moltiplicatore_size": 1.0 se il quadro e\' pulito o comunque coerente con il '
        f"trade; scendi verso 0.5-0.7 se ci sono elementi di incertezza reale (evento in "
        f"arrivo ma non imminente, notizie miste, volatilita' elevata) senza arrivare al veto\n"
        f"- Non cercare la perfezione: il compito e' evitare i casi evidentemente rischiosi "
        f"e dimensionare meglio quelli incerti, non prevedere il futuro"
    )
    ...
    try:
        ...
        inizio, fine = testo_pulito.find("{"), testo_pulito.rfind("}")
        if inizio == -1 or fine == -1:
            logger.warning(f"{segnale.coppia}: risposta IA approfondita senza JSON valido, procedo con size piena. Risposta: {testo[:300]}")
            return True, 1.0, "risposta IA non interpretabile - trade eseguito con size piena"
        verdetto = json.loads(testo_pulito[inizio:fine + 1])
        procedere = bool(verdetto.get("procedere", True))
        try:
            moltiplicatore = float(verdetto.get("moltiplicatore_size", 1.0))
        except (TypeError, ValueError):
            moltiplicatore = 1.0
        moltiplicatore = max(MOLTIPLICATORE_SIZE_MINIMO_IA, min(1.0, moltiplicatore))
        motivo = str(verdetto.get("motivo", ""))[:500]

        # --- Confronto prezzo Bitget vs prezzo verificato da fonte esterna ---
        # Questo controllo e' fatto qui in Python (non lasciato solo al giudizio
        # dell'IA) cosi' la soglia e' oggettiva e verificabile, non dipende
        # dall'interpretazione del modello.
        if richiede_verifica_prezzo and verdetto.get("prezzo_verificato") is not None:
            try:
                prezzo_esterno = float(verdetto["prezzo_verificato"])
                divergenza_percento = abs(prezzo_esterno - segnale.entry) / segnale.entry * 100
                if divergenza_percento >= SOGLIA_DIVERGENZA_PREZZO_VETO_PERCENTO:
                    procedere = False
                    motivo = (
                        f"Prezzo Bitget ({segnale.entry}) disallineato del {divergenza_percento:.2f}% "
                        f"dal prezzo reale verificato ({prezzo_esterno}) - trade annullato per "
                        f"rischio di tracking errato dello strumento tokenizzato. {motivo}"
                    )
                elif divergenza_percento >= SOGLIA_DIVERGENZA_PREZZO_PERCENTO:
                    moltiplicatore = min(moltiplicatore, 0.6)
                    motivo = (
                        f"Prezzo Bitget ({segnale.entry}) scostato del {divergenza_percento:.2f}% "
                        f"dal prezzo reale verificato ({prezzo_esterno}) - size ridotta per "
                        f"prudenza. {motivo}"
                    )
            except (TypeError, ValueError):
                logger.info(f"{segnale.coppia}: campo prezzo_verificato non numerico, ignorato: {verdetto.get('prezzo_verificato')}")

        logger.info(
            f"{segnale.coppia}: analisi IA approfondita (LIVE) -> "
            f"{'PROCEDI' if procedere else 'VETO'}, size x{moltiplicatore:.2f} ({motivo})"
        )
        return procedere, moltiplicatore, motivo
    except Exception as e:
        logger.warning(f"{segnale.coppia}: analisi IA approfondita fallita ({e}) - procedo con size piena senza questo filtro")
        return True, 1.0, f"analisi approfondita non disponibile: {e}"
...
async def esegui_apertura_trade(bot: Bot, segnale: Segnale) -> dict | None:
    ...
    # Guardia di sicurezza extra: analizza_coppia() ha gia' scartato le azioni fuori
    # orario PRIMA di generare il segnale, ma tra la generazione e l'apertura vera e
    # propria passa del tempo (chiamate API, eventuale analisi IA approfondita in LIVE)
    # - se il mercato ha chiuso proprio in quella finestra, non apriamo comunque.
    if (CONFIG.get("rispetta_orari_azioni", True)
            and categoria_richiede_orario_di_borsa(segnale.coppia)
            and not mercato_azionario_aperto()):
        logger.info(f"{segnale.coppia}: mercato azionario chiuso nel frattempo, trade non eseguito.")
        return None
    ...
    # --- Analisi di mercato/notizie PRIMA di calcolare la size, cosi' il suo verdetto
    # (procedere si'/no, ed eventuale riduzione size) puo' effettivamente influenzare
    # il trade. In LIVE (soldi reali) usiamo la versione approfondita (piu' ricerche,
    # contesto per categoria di mercato, puo' ridurre la size invece di solo vetare).
    # In Demo resta il controllo leggero originale, puramente si'/no. Aggiunto/esteso
    # 05/09/2026 su richiesta di Fabio. IMPORTANTE: resta un filtro di rischio in piu',
    # non una garanzia di profitto - nessuna analisi puo' prevedere il mercato con certezza.
    moltiplicatore_ia = 1.0
    if bitget_demo_attivo():
        procedere, motivo_ia = await parere_ia_su_segnale(segnale)
    else:
        procedere, moltiplicatore_ia, motivo_ia = await analisi_approfondita_live(segnale)
    if not procedere:
        ...
        await notifica_admin(
            ...
            f"{' (analisi approfondita LIVE)' if not bitget_demo_attivo() else ''}:\n"
            f"_{motivo_ia}_"
        )
        return None
    if not bitget_demo_attivo() and moltiplicatore_ia < 1.0:
        await notifica_admin(
            bot,
            f"🔎 Analisi approfondita su `{segnale.coppia}`: size ridotta al "
            f"{moltiplicatore_ia*100:.0f}% di quella prevista dal rischio configurato.\n"
            f"_{motivo_ia}_"
        )

    margine_usdt = saldo * (CONFIG["rischio_percento"] / 100) * moltiplicatore_ia
...
def adx(df: pd.DataFrame, periodo: int = 14) -> pd.Series:
    """Average Directional Index (Wilder): misura QUANTO e' forte un trend, non la sua
    direzione. Sotto ~20 il mercato e' tipicamente laterale, sopra ~25 e' direzionale.
    Usato solo per capire in che condizione si trova il mercato e suggerire la
    strategia piu' adatta - non entra nel punteggio dei segnali."""
    up = df["high"].diff()
    down = -df["low"].diff()
...
# ---------------------------------------------------------------------------
# STRATEGIE (Expert Advisor selezionabili dalla dashboard o da /strategia)
# ---------------------------------------------------------------------------
# Cinque strategie classiche e documentate. NESSUNA e' "la migliore": ognuna funziona
# in certe condizioni di mercato e fallisce in altre. Il campo "regime" dice in quale
# condizione tende a rendere meglio secondo la letteratura sull'analisi tecnica - e'
# un'indicazione generale, non un risultato misurato sul conto di Fabio. Le statistiche
# reali per strategia (statistiche["per_strategia"]) servono proprio a verificarlo.
# Tutte condividono: filtro trend 4h (EMA50/200 + prezzo), conferma 15m, filtro
# anti-mercato-piatto, gestione SL/TP con i moltiplicatori ATR della dashboard.

STRATEGIE = {
    "completo": {
        "nome": "Trend following",
        "descrizione": "Trend 4h + RSI + MACD + volume. Selettiva, pochi segnali ben confermati.",
        "funziona_quando": "mercato in trend deciso",
        "fallisce_quando": "mercato laterale: i segnali arrivano tardi",
        "regime": "trend",
    },
    "rsi_semplice": {
        "nome": "Mean reversion (RSI)",
        "descrizione": "Compra i ribassi (vende i rialzi) dentro un trend confermato: RSI uscito da ipervenduto/ipercomprato.",
        "funziona_quando": "mercato che oscilla attorno a un trend moderato",
        "fallisce_quando": "trend fortissimi o crolli: l'ipervenduto puo' durare a lungo",
        "regime": "laterale",
    },
    "breakout": {
        "nome": "Breakout di volatilita'",
        "descrizione": "Entra quando il prezzo rompe il massimo/minimo delle ultime 20 ore con volume forte.",
        "funziona_quando": "mercato che esce da una fase di compressione",
        "fallisce_quando": "falsi breakout in mercati senza direzione",
        "regime": "transizione",
    },
    "incrocio_medie": {
        "nome": "Incrocio medie mobili",
        "descrizione": "EMA9 incrocia EMA21 su 1h nella direzione del trend 4h. Semplice e leggibile.",
        "funziona_quando": "trend che riprende dopo una pausa",
        "fallisce_quando": "mercato laterale: incroci continui e falsi",
        "regime": "trend",
    },
    "momentum_mtf": {
        "nome": "Momentum multi-timeframe",
        "descrizione": "4h, 1h e 15m tutti allineati, RSI in zona di forza (non estrema), MACD a favore.",
        "funziona_quando": "trend forte e ordinato",
        "fallisce_quando": "inversioni improvvise: entra quando il movimento e' gia' avviato",
        "regime": "trend",
    },
}

SOGLIA_ADX_TREND = 25    # sopra: mercato direzionale
SOGLIA_ADX_LATERALE = 20  # sotto: mercato laterale; in mezzo: fase di transizione
MIN_TRADE_PER_STATISTICA_AFFIDABILE = 10


def regime_da_adx(valore_adx) -> str:
    if valore_adx is None or pd.isna(valore_adx):
        return "sconosciuto"
    if valore_adx >= SOGLIA_ADX_TREND:
        return "trend"
    if valore_adx < SOGLIA_ADX_LATERALE:
        return "laterale"
    return "transizione"


def suggerimento_strategia(statistiche: dict = None) -> dict:
    """Guarda il regime (ADX su 4h) di tutti i mercati attivi appena analizzati e
    suggerisce quale strategia si adatta alla condizione prevalente. Onesto per
    costruzione: dice su quanti mercati si basa, e affianca i risultati REALI per
    strategia quando ce ne sono abbastanza - la teoria da sola non basta."""
    conteggio = {"trend": 0, "transizione": 0, "laterale": 0}
    valori_adx = []
    piatti = 0
    for pair in watchlist_attiva():
        d = MONITORAGGIO.get(pair) or {}
        reg = d.get("regime")
        if reg in conteggio:
            conteggio[reg] += 1
            if d.get("adx") is not None:
                valori_adx.append(d["adx"])
        if d.get("esito") == "mercato_piatto":
            piatti += 1
    analizzati = sum(conteggio.values())
    risultato = {
        "mercati_analizzati": analizzati,
        "conteggio_regimi": conteggio,
        "adx_mediano": round(float(np.median(valori_adx)), 1) if valori_adx else None,
        "regime_prevalente": None,
        "strategia_consigliata": None,
        "alternativa": None,
        "motivo": "Servono i dati della prima scansione per suggerire una strategia.",
        "cautela": None,
    }
    if analizzati == 0:
        return risultato

    regime = max(conteggio, key=conteggio.get)
    risultato["regime_prevalente"] = regime
    quota = round(conteggio[regime] / analizzati * 100)
    if regime == "trend":
        risultato["strategia_consigliata"] = "completo"
        risultato["alternativa"] = "momentum_mtf"
        risultato["motivo"] = (f"{quota}% dei mercati attivi e' in trend deciso (ADX >= {SOGLIA_ADX_TREND}): "
                               "le strategie che seguono il trend tendono a rendere meglio.")
    elif regime == "laterale":
        risultato["strategia_consigliata"] = "rsi_semplice"
        risultato["alternativa"] = None
        risultato["motivo"] = (f"{quota}% dei mercati attivi e' laterale (ADX < {SOGLIA_ADX_LATERALE}): "
                               "in assenza di trend, comprare i ribassi e vendere i rialzi tende a funzionare meglio "
                               "che inseguire la direzione.")
        risultato["cautela"] = ("In mercato laterale il filtro trend del bot scarta molti segnali: "
                                "e' normale vedere poca operativita'.")
    else:
        risultato["strategia_consigliata"] = "breakout"
        risultato["alternativa"] = "incrocio_medie"
        risultato["motivo"] = (f"{quota}% dei mercati attivi e' in fase di transizione (ADX tra "
                               f"{SOGLIA_ADX_LATERALE} e {SOGLIA_ADX_TREND}): spesso precede l'uscita da un range.")
        risultato["cautela"] = "I falsi breakout sono frequenti: il volume forte e' la conferma che conta."

    if piatti > analizzati / 2:
        risultato["cautela"] = (f"{piatti} mercati su {analizzati} hanno volatilita' sotto la soglia: "
                                "qualunque strategia avra' pochi segnali ora. Valuta di attendere.")

    # Affianca i dati reali, se ce ne sono abbastanza da dire qualcosa.
    per_strategia = (statistiche or {}).get("per_strategia", {})
    reali = []
    for chiave, s in per_strategia.items():
        tot = s.get("vinti", 0) + s.get("persi", 0) + s.get("pareggi", 0)
        if tot >= MIN_TRADE_PER_STATISTICA_AFFIDABILE:
            reali.append((s.get("pnl_usd", 0.0), chiave, tot))
    if reali:
        reali.sort(reverse=True)
        pnl, chiave, tot = reali[0]
        risultato["migliore_sui_tuoi_dati"] = {"strategia": chiave, "trade": tot, "pnl_usd": round(pnl, 2)}
    return risultato


def aggiorna_statistiche_strategia(statistiche: dict, strategia: str, esito: str, pnl_usd: float):
    riga = statistiche.setdefault("per_strategia", {}).setdefault(
        strategia, {"vinti": 0, "persi": 0, "pareggi": 0, "pnl_usd": 0.0}
    )
    if esito == "perso":
        riga["persi"] += 1
    elif esito == "pareggio":
        ...
        riga["vinti"] += 1
    riga["pnl_usd"] = round(riga.get("pnl_usd", 0.0) + (pnl_usd or 0.0), 4)


def valuta_strategia_extra(strategia: str, df_1h: pd.DataFrame, bias: "Direzione",
                           ultimo, bonus_soglia: int = 0) -> dict:
    """Condizioni d'ingresso per breakout / incrocio_medie / momentum_mtf.
    Ritorna {"score", "soglia", "valido", "motivi", "condizioni"}.
    Il trend 4h (30 punti) e' gia' verificato a monte. Le condizioni "richiesta" devono
    essere tutte vere; quelle facoltative aggiungono punti e servono quando lo studio di
    mercato alza la prudenza (bonus_soglia > 0)."""
    long = bias == Direzione.LONG
    condizioni = []  # (nome, ok, punti, richiesta, motivo)

    if strategia == "breakout":
        finestra = df_1h.iloc[-21:-1]  # le 20 ore prima dell'ultima candela
        livello = finestra["high"].max() if long else finestra["low"].min()
        rotto = (ultimo["close"] > livello) if long else (ultimo["close"] < livello)
        vol_forte = pd.notna(ultimo["volume_media"]) and ultimo["volume"] > ultimo["volume_media"] * 1.5
        # Non inseguire un breakout gia' esteso: il prezzo deve essere entro 1 ATR dal livello rotto.
        non_esteso = pd.notna(ultimo["atr"]) and abs(ultimo["close"] - livello) <= ultimo["atr"]
        condizioni = [
            ("Rottura 20h", bool(rotto), 35, True,
             f"Rottura del {'massimo' if long else 'minimo'} delle ultime 20 ore"),
            ("Volume x1.5", bool(vol_forte), 25, True, "Volume oltre 1.5 volte la media"),
            ("Non esteso", bool(non_esteso), 10, False, "Prezzo ancora vicino al livello rotto"),
        ]
    elif strategia == "incrocio_medie":
        df_1h["ema9"] = ema(df_1h["close"], 9)
        df_1h["ema21"] = ema(df_1h["close"], 21)
        n = max(1, int(CONFIG.get("finestra_macd_candele", 3)))
        incrocio = False
        for i in range(1, n + 1):
            prec_i, cur_i = df_1h.iloc[-i - 1], df_1h.iloc[-i]
            if long and prec_i["ema9"] <= prec_i["ema21"] and cur_i["ema9"] > cur_i["ema21"]:
                incrocio = True
                break
            if not long and prec_i["ema9"] >= prec_i["ema21"] and cur_i["ema9"] < cur_i["ema21"]:
                incrocio = True
                break
        ultimo_ema = df_1h.iloc[-1]
        lato_giusto = (ultimo_ema["close"] > ultimo_ema["ema21"]) if long else (ultimo_ema["close"] < ultimo_ema["ema21"])
        rsi_ok = pd.notna(ultimo["rsi"]) and ((45 <= ultimo["rsi"] <= 65) if long else (35 <= ultimo["rsi"] <= 55))
        condizioni = [
            ("Incrocio EMA9/21", bool(incrocio), 40, True,
             f"EMA9 ha incrociato EMA21 {'al rialzo' if long else 'al ribasso'} nelle ultime {n} ore"),
            ("Prezzo oltre EMA21", bool(lato_giusto), 20, True, "Prezzo dalla parte giusta della EMA21"),
            ("RSI non estremo", bool(rsi_ok), 10, False, "RSI non gia' in eccesso"),
        ]
    elif strategia == "momentum_mtf":
        df_1h["ema21"] = ema(df_1h["close"], 21)
        ultimo_ema = df_1h.iloc[-1]
        lato_giusto = (ultimo_ema["close"] > ultimo_ema["ema21"]) if long else (ultimo_ema["close"] < ultimo_ema["ema21"])
        rsi_forza = pd.notna(ultimo["rsi"]) and ((55 <= ultimo["rsi"] <= 70) if long else (30 <= ultimo["rsi"] <= 45))
        macd_favore = (ultimo["macd"] > ultimo["macd_signal"]) if long else (ultimo["macd"] < ultimo["macd_signal"])
        vol_ok = pd.notna(ultimo["volume_media"]) and ultimo["volume"] > ultimo["volume_media"]
        condizioni = [
            ("Prezzo oltre EMA21 1h", bool(lato_giusto), 20, True, "Trend 1h allineato al 4h"),
            ("RSI in zona di forza", bool(rsi_forza), 25, True,
             "RSI in zona di forza " + ("55-70" if long else "30-45")),
            ("MACD a favore", bool(macd_favore), 15, True, "MACD sopra la linea di segnale" if long else "MACD sotto la linea di segnale"),
            ("Volume sopra media", bool(vol_ok), 10, False, "Volume sopra la media"),
        ]
    else:
        raise ValueError(f"Strategia sconosciuta: {strategia}")

    score = 30 + sum(punti for _, ok, punti, _, _ in condizioni if ok)
    soglia = 30 + sum(punti for _, _, punti, richiesta, _ in condizioni if richiesta)
    if bonus_soglia > 0:
        # Prudenza alta dallo studio di mercato: servono anche le condizioni facoltative.
        soglia = 30 + sum(punti for _, _, punti, _, _ in condizioni)
    richieste_ok = all(ok for _, ok, _, richiesta, _ in condizioni if richiesta)
    return {
        "score": score,
        "soglia": soglia,
        "valido": richieste_ok and score >= soglia,
        "motivi": [motivo for _, ok, _, _, motivo in condizioni if ok],
        "condizioni": [{"nome": nome, "ok": ok} for nome, ok, _, _, _ in condizioni],
    }
...
# --- Soglie calibrate per categoria (04/09/2026, ricalibrate 05/09/2026 per azioni e
# materie prime dato che sono ora le categorie primarie di Fabio) ---
# Motivo generale: una soglia unica di volatilita' non ha senso su mercati cosi' diversi.
# L'oro si muove tipicamente 0.5-1% al giorno e le azioni anche meno, quindi con lo 0.35%
# pensato per le crypto verrebbero scartati quasi sempre come "mercato piatto" anche con
# setup validi. Le meme coin invece oscillano del 10-15%: per loro 0.35% e' troppo
# permissivo e lascia passare rumore. Stesso ragionamento per la distanza minima fra le EMA.
...
#
# Ricalibrazione azioni/materie_prime (05/09/2026): alzate leggermente rispetto a
# 0.15/0.3 - quel valore era pensato solo per non scartare l'oro/le azioni come "piatti",
# ma era troppo permissivo come filtro di qualita' del trend (vedi diagnosi win rate
# 19.6%: soglie troppo larghe fanno entrare su rumore scambiato per trend). Materie
# prime tenute leggermente piu' larghe delle azioni pure per via di petrolio/gas
# naturale, piu' soggetti a spike improvvisi rispetto a un paniere di azioni large-cap.
#
# Ricalibrazione crypto_major (05/09/2026): ema_min alzato da 0.5% a 1.2% - causa
# diretta identificata del win rate reale 19.6% su 204 trade (164 persi): con lo 0.5%
# bastava un incrocio EMA50/200 marginale, indistinguibile dal rumore normale di
# oscillazione, per far scattare un "trend confermato". Il prezzo tornava indietro
# quasi subito nella maggioranza dei casi, colpendo lo Stop Loss senza mai avvicinarsi
# al primo target. 1.2% richiede una separazione delle medie molto piu' netta prima di
# fidarsi della direzione - meno segnali, ma su trend che hanno gia' mostrato di reggere.
#
# Questi sono i valori DI FABBRICA - la dashboard puo' sovrascriverli per singola
# categoria tramite CONFIG["soglie_categorie"] (vedi soglie_di()), senza toccare
# il codice. Facoltativo: il bot parte gia' con questi valori, li cambi solo se vuoi.
SOGLIE_PER_CATEGORIA_DEFAULT = {
    "crypto_major":  {"atr_min": 0.40, "ema_min": 1.2},
    "azioni":        {"atr_min": 0.20, "ema_min": 0.45},
    "etf":           {"atr_min": 0.12, "ema_min": 0.35},
    "materie_prime": {"atr_min": 0.25, "ema_min": 0.55},
    ...
    # "stablecoin" non compare qui apposta: non passa mai per soglie_di(), vedi il
    # controllo dedicato controlla_depeg_stablecoin().
}
...
def soglie_di(pair: str) -> dict:
    """Soglie atr_min/ema_min per la categoria di questo simbolo. Priorita': override
    salvato dalla dashboard (CONFIG["soglie_categorie"]) -> valore di fabbrica
    (SOGLIE_PER_CATEGORIA_DEFAULT) -> fallback crypto_major se la categoria e' nuova
    e non ha ancora un default definito."""
    chiave = categoria_di(pair)
    default = SOGLIE_PER_CATEGORIA_DEFAULT.get(chiave, SOGLIE_PER_CATEGORIA_DEFAULT["crypto_major"])
    override = CONFIG.get("soglie_categorie", {}).get(chiave, {})
    return {
        "atr_min": override.get("atr_min", default["atr_min"]),
        "ema_min": override.get("ema_min", default["ema_min"]),
    }


# --- Orario di apertura di Wall Street (05/09/2026, su richiesta di Fabio) ---
# Le "azioni" quotate su Bitget sono strumenti tokenizzati che replicano il prezzo
# REALE della borsa USA (NYSE/NASDAQ). Fuori dall'orario di contrattazione il prezzo
# di riferimento e' fermo (o si muove pochissimo sul pre/post market) - aprire un
# trade li' significa entrare su un prezzo stantio, con lo Stop Loss/Take Profit
# calcolati su un ATR che riflette la volatilita' del mercato APERTO, non di quello
# fermo. Il controllo copre l'orario regolare 9:30-16:00 orario di New York, dal
# lunedi' al venerdi'.
# LIMITE NOTO (documentato onestamente): questo controllo NON conosce le festivita'
# della borsa USA (es. Independence Day, Thanksgiving) - in quei giorni pensera' che
# il mercato sia aperto quando in realta' e' chiuso. Un calendario festivita' completo
# richiederebbe una libreria dedicata o una lista aggiornata a mano ogni anno; per ora
# resta un limite noto, non silenzioso.
FUSO_WALL_STREET = ZoneInfo("America/New_York")
ORARIO_APERTURA_WALL_STREET = (9, 30)   # 9:30 orario di New York
ORARIO_CHIUSURA_WALL_STREET = (16, 0)   # 16:00 orario di New York


def mercato_azionario_aperto() -> bool:
    """True se Wall Street e' aperta ORA (orario regolare, lun-ven). Non tiene conto
    delle festivita' di borsa - vedi limite documentato sopra la funzione."""
    ora_ny = datetime.now(FUSO_WALL_STREET)
    if ora_ny.weekday() >= 5:  # 5=sabato, 6=domenica
        return False
    apertura = ora_ny.replace(hour=ORARIO_APERTURA_WALL_STREET[0], minute=ORARIO_APERTURA_WALL_STREET[1], second=0, microsecond=0)
    chiusura = ora_ny.replace(hour=ORARIO_CHIUSURA_WALL_STREET[0], minute=ORARIO_CHIUSURA_WALL_STREET[1], second=0, microsecond=0)
    return apertura <= ora_ny < chiusura


def categoria_richiede_orario_di_borsa(pair: str) -> bool:
    """Azioni ed ETF seguono l'orario di Wall Street (gli ETF replicano titoli quotati
    in borsa esattamente come le singole azioni). Le materie prime hanno anch'esse
    orari di borsa (es. COMEX, NYMEX) ma spesso quasi 24/5 e diversi da asset ad asset
    - lasciate fuori da questo controllo per ora, valutabile in futuro se Fabio nota
    lo stesso problema anche li'."""
    return categoria_di(pair) in ("azioni", "etf")
...
def soglia_score_dinamica(statistiche: dict, pair: str = None) -> int:
    perdite = statistiche.get("perdite_consecutive", 0) if statistiche else 0
    # Disattivabile dalla dashboard (05/09/2026, su richiesta di Fabio) - di default
    # ogni perdita consecutiva alza la soglia (protezione anti "revenge trading": non
    # abbassare la selettivita' proprio subito dopo aver perso). Fabio ha scelto di
    # disattivarlo: la soglia resta fissa a CONFIG["score_minimo"] indipendentemente
    # dalle perdite consecutive.
    if CONFIG.get("escalation_soglia_dopo_perdite_attiva", False):
        bonus = min(perdite * BONUS_SOGLIA_PER_PERDITA, BONUS_SOGLIA_MASSIMO)
    else:
        bonus = 0
    score_base = CONFIG.get("score_minimo", SCORE_MINIMO_PUBBLICAZIONE_DEFAULT)
    # Bonus extra dallo studio generale di mercato per la categoria di questo pair (vedi
    # aggiorna_contesto_mercato_categoria) - una giornata di forte incertezza per una
    # categoria alza temporaneamente la soglia richiesta solo li', non ovunque.
    bonus_mercato = 0
    if pair is not None:
        contesto = _CONTESTO_MERCATO_CATEGORIA.get(categoria_di(pair))
        if contesto:
            bonus_mercato = contesto.get("bonus_soglia", 0)
    return score_base + bonus + bonus_mercato


# --- Monitoraggio depeg per le stablecoin (05/09/2026, su richiesta di Fabio) ---
# Le stablecoin non passano per il sistema di score trend-following: per definizione
# dovrebbero restare ancorate a 1 dollaro, quindi EMA/RSI/MACD non hanno alcun senso
# su di loro. Qui controlliamo solo quanto il prezzo si discosta dal peg (1.00 USD).
#
# SCELTA IMPORTANTE, spiegata a Fabio: questo e' SOLO un monitoraggio con allerta
# Telegram, MAI un'apertura automatica di posizione a leva. Un depeg puo' essere un
# rimbalzo temporaneo (es. USDC nel marzo 2023, tornato a 1.00 in poche ore) oppure
# PERMANENTE (es. UST/Terra nel 2022, mai tornata al peg, azzerata) - dato che non c'e'
# modo affidabile di distinguere i due casi nel momento in cui il depeg comincia, uno
# Stop Loss a leva non protegge da uno scenario UST: il prezzo puo' saltare oltre lo SL
# prima che l'ordine si esegua, o la liquidita' puo' sparire del tutto. Meglio
# un'allerta che avvisa Fabio e lascia a lui la decisione, che un'apertura automatica
# su un asset il cui comportamento in caso di rottura del peg e' imprevedibile.
SOGLIA_DEPEG_ALLERTA_PERCENTO = 0.5   # scostamento dal peg (in %) che fa scattare l'allerta
COOLDOWN_ALLERTA_DEPEG_ORE = 4        # non ripetere l'allerta sulla stessa stablecoin entro N ore
_ULTIMA_ALLERTA_DEPEG: dict = {}      # pair -> datetime dell'ultima allerta inviata (solo in memoria)


def controlla_depeg_stablecoin(pair: str) -> dict:
    """Controlla di quanto il prezzo di una stablecoin si discosta da 1.00 USD.
    Ritorna un dizionario per MONITORAGGIO, mai un Segnale - vedi la nota sopra sul
    perche' questo resta volutamente solo un'allerta e non un trade automatico."""
    dettagli = {
        "pair": pair, "score": 0, "direzione": "", "esito": "errore_dati",
        "prezzo": None, "atr_percento": None, "rsi_ok": None, "macd_ok": None,
        "volume_ok": None, "soglia_attuale": None, "deviazione_percento": None,
        "aggiornato": datetime.now().isoformat(),
    }
    try:
        prezzo = get_prezzo_per_monitoraggio(pair)
    except Exception as e:
        logger.warning(f"{pair}: impossibile leggere il prezzo per il controllo depeg: {e}")
        return dettagli

    deviazione = (prezzo - 1.0) * 100  # percento, positivo=sopra il peg, negativo=sotto
    dettagli["prezzo"] = prezzo
    dettagli["deviazione_percento"] = round(deviazione, 3)
    if abs(deviazione) >= SOGLIA_DEPEG_ALLERTA_PERCENTO:
        dettagli["esito"] = "depeg_rilevato"
        dettagli["direzione"] = "sopra il peg" if deviazione > 0 else "sotto il peg"
    else:
        dettagli["esito"] = "peg_stabile"
    return dettagli
...
def analizza_coppia(pair: str, statistiche: dict = None) -> tuple[Segnale | None, int, str]:
    dettagli = {
        ...
        "volume_ok": False, "soglia_attuale": soglia_score_dinamica(statistiche, pair),
        "aggiornato": datetime.now().isoformat(),
        "adx": None, "regime": None, "condizioni": None,
        "strategia": CONFIG.get("modalita_segnale", "completo"),
    }

    # --- Stablecoin: bypass completo del sistema di score, vedi controlla_depeg_stablecoin().
    # Ritorna sempre None come segnale - non apre mai un trade automaticamente.
    if categoria_di(pair) == "stablecoin":
        dettagli_depeg = controlla_depeg_stablecoin(pair)
        MONITORAGGIO[pair] = dettagli_depeg
        return None, 0, ""

    # --- Orario di borsa (solo categoria "azioni", vedi mercato_azionario_aperto()) ---
    # Controllo messo qui, PRIMA di scaricare dati e calcolare qualunque score: fuori
    # orario non ha senso nemmeno analizzare la coppia, il prezzo di riferimento e'
    # fermo. Facoltativo tramite CONFIG["rispetta_orari_azioni"] (default: attivo).
    if (CONFIG.get("rispetta_orari_azioni", True)
            and categoria_richiede_orario_di_borsa(pair)
            and not mercato_azionario_aperto()):
        dettagli["esito"] = "mercato_chiuso"
        dettagli["score"] = 0
    ...
    # Regime di mercato (ADX 4h): serve solo al suggerimento di strategia, calcolato qui
    # prima di ogni scarto cosi' e' disponibile per tutti i mercati analizzati.
    try:
        valore_adx = adx(df_4h).iloc[-1]
        if pd.notna(valore_adx):
            dettagli["adx"] = round(float(valore_adx), 1)
            dettagli["regime"] = regime_da_adx(valore_adx)
    except Exception as e:
        logger.info(f"{pair}: ADX non calcolabile ({e})")
    ...
    n_candele_rsi = max(1, int(CONFIG.get("finestra_rsi_candele", 5)))
    finestra_rsi = df_1h["rsi"].iloc[-(n_candele_rsi + 1):-1]  # le N candele prima dell'ultima, N da dashboard
    ...
    if modalita_segnale in ("breakout", "incrocio_medie", "momentum_mtf"):
        # Strategie aggiunte il 19/09/2026 (su richiesta di Fabio). Hanno condizioni
        # proprie: azzeriamo il punteggio RSI calcolato sopra e ripartiamo dal trend.
        bonus_mercato = _CONTESTO_MERCATO_CATEGORIA.get(categoria_di(pair), {}).get("bonus_soglia", 0)
        esito_strategia = valuta_strategia_extra(modalita_segnale, df_1h, bias, ultimo, bonus_mercato)
        score = esito_strategia["score"]
        motivi = [motivi[0]] + esito_strategia["motivi"]
        dettagli["rsi_ok"] = None
        dettagli["macd_ok"] = None
        dettagli["volume_ok"] = None
        dettagli["condizioni"] = esito_strategia["condizioni"]
        dettagli["score"] = score
        dettagli["soglia_attuale"] = esito_strategia["soglia"]
        if not esito_strategia["valido"]:
            logger.info(f"{pair}: [{modalita_segnale}] score {score}/{esito_strategia['soglia']} ({bias.value}) - condizioni non soddisfatte, scartato")
            dettagli["esito"] = "sotto_soglia"
            MONITORAGGIO[pair] = dettagli
            return None, score, bias.value
        logger.info(f"{pair}: [{modalita_segnale}] score {score}/100 ({bias.value}) - SEGNALE VALIDO")
    elif modalita_segnale == "rsi_semplice":
        ...
        dettagli["score"] = score
        bonus_mercato = _CONTESTO_MERCATO_CATEGORIA.get(categoria_di(pair), {}).get("bonus_soglia", 0)
        soglia_attuale = 55 + bonus_mercato  # in questa modalita' serve comunque trend+RSI insieme (30+25)
        ...
        # basta sia avvenuto entro N ore (CONFIG["finestra_macd_candele"], modificabile
        # da dashboard) e il MACD sia ancora coerente con la direzione adesso.
        macd_ok = False
        n_candele_macd = max(1, int(CONFIG.get("finestra_macd_candele", 3)))
        for i in range(1, n_candele_macd + 1):
            prec_i, cur_i = df_1h.iloc[-i - 1], df_1h.iloc[-i]
        ...
        if score < soglia_attuale:
            extra_info = f" (soglia alzata a {soglia_attuale} per {statistiche.get('perdite_consecutive', 0)} perdite consecutive)" if soglia_attuale > CONFIG.get("score_minimo", SCORE_MINIMO_PUBBLICAZIONE_DEFAULT) else ""
    ...
    # SL invariato a 2.0x ATR. TP riavvicinati (04/09/2026): con i precedenti 2.0/4.5/8.0
    # il target finale cadeva a +10-14% sulle coppie reali - un movimento che raramente
    # avviene entro il timeout di 24h, rendendo TP3 di fatto irraggiungibile. Inoltre TP1
    # era alla STESSA distanza dello SL (50% grezzo di toccare l'uno o l'altro): ora e' a
    # 1.2x contro SL 2.0x, quindi si raggiunge molto prima, lo SL passa a breakeven presto
    # e il trade smette di poter perdere. R:R finale 1:2.25 - piu' basso di prima, ma su
    # target realmente raggiungibili invece che teorici.
    mult_sl = float(CONFIG.get("atr_moltiplicatore_sl", 2.0))
    mult_tp1 = float(CONFIG.get("atr_moltiplicatore_tp1", 1.2))
    mult_tp2 = float(CONFIG.get("atr_moltiplicatore_tp2", 2.5))
    mult_tp3 = float(CONFIG.get("atr_moltiplicatore_tp3", 4.5))
    if bias == Direzione.LONG:
        stop_loss = entry - (mult_sl * moltiplicatore_volatilita * atr_val)
        tp1 = entry + (mult_tp1 * moltiplicatore_volatilita * atr_val)
        tp2 = entry + (mult_tp2 * moltiplicatore_volatilita * atr_val)
        tp3 = entry + (mult_tp3 * moltiplicatore_volatilita * atr_val)
    else:
        stop_loss = entry + (mult_sl * moltiplicatore_volatilita * atr_val)
        tp1 = entry - (mult_tp1 * moltiplicatore_volatilita * atr_val)
        tp2 = entry - (mult_tp2 * moltiplicatore_volatilita * atr_val)
...
def formatta_messaggio(s: Segnale, eseguito: bool, non_supportata_in_demo: bool = False) -> str:
    ...
    return (
        ...
        f"*Strategia:* {STRATEGIE.get(s.strategia, {}).get('nome', s.strategia)}\n"
...
async def controlla_posizioni_aperte(bot: Bot, posizioni_aperte: dict, statistiche: dict):
    ...
    for pair, pos in list(posizioni_aperte.items()):
        ...
        if size_attuale <= SOGLIA_CHIUSURA_RESIDUA or size_attuale < get_info_contratto(pair)["min_trade_num"] * 0.5:
            ...
            # Verifica sui dati reali di Bitget: se la posizione e' stata chiusa a mano
            # (o in qualunque modo diverso da quello ipotizzato sopra), l'esito vero lo
            # dice il profitto netto realmente incassato, non gli ordini piano.
            chiusura_reale = None
            if not pos.get("chiusura_forzata"):
                chiusura_reale = get_chiusura_reale(pair, pos.get("aperto_il"))
            if chiusura_reale is not None:
                pnl_reale = chiusura_reale["pnl"]
                tolleranza_pareggio = 0.01  # USDT: sotto questa cifra e' di fatto un pareggio
                if pnl_reale > tolleranza_pareggio:
                    esito_reale = "vinto_parziale"
                elif pnl_reale < -tolleranza_pareggio:
                    esito_reale = "perso"
                else:
                    esito_reale = "pareggio"
                if esito_reale != esito:
                    logger.info(
                        f"{pair}: esito corretto sui dati reali di Bitget - ipotizzato '{esito}', "
                        f"reale '{esito_reale}' (profitto netto {pnl_reale:+.4f} USDT)."
                    )
                    if not pos["tp1_raggiunto"] and not pos["tp2_raggiunto"]:
                        titolo = {
                            "vinto_parziale": "✅ *POSIZIONE CHIUSA IN PROFITTO*",
                            "pareggio": "➖ *POSIZIONE CHIUSA IN PAREGGIO*",
                            "perso": "🛑 *POSIZIONE CHIUSA IN PERDITA*",
                        }[esito_reale]
                        corpo = (
                            f"Posizione su `{pair}` chiusa senza che scattassero i target del bot "
                            f"(chiusura manuale dall'app, oppure Stop Loss)."
                        )
                    esito = esito_reale
            ...
            if chiusura_reale is not None:
                # Numeri di Bitget: profitto netto (commissioni incluse) e prezzo medio
                # di chiusura effettivo. Sono la verita' sul trade, meglio di ogni stima.
                pnl_usd = chiusura_reale["pnl"]
                if chiusura_reale["prezzo_chiusura"]:
                    prezzo_chiusura = chiusura_reale["prezzo_chiusura"]
            ...
            aggiorna_statistiche_strategia(statistiche, getattr(s, "strategia", "completo"), esito, pnl_usd)
...
def _messaggio_benvenuto() -> str:
    ...
    return (
        ...
        f"e pubblica solo i setup con punteggio di confidenza ≥{CONFIG.get('score_minimo', SCORE_MINIMO_PUBBLICAZIONE_DEFAULT)}/100.\n\n"
        f"{riga_trading}"
        f"{riga_limite_demo}\n"
...
async def ciclo_scansione(bot: Bot, ultimo_segnale: dict, posizioni_aperte: dict, statistiche: dict, storia_segnali: list):
    ...
    # --- FASE 0: studio generale di mercato per categoria (05/09/2026) ---
    # Aggiorna (se la cache e' scaduta) lo stato generale delle categorie "grandi"
    # (azioni, ETF, materie prime, crypto major) da fonti ESTERNE a Bitget, PRIMA di
    # analizzare qualunque singolo mercato - vedi aggiorna_contesto_mercato_categoria().
    # Una sola chiamata per categoria attiva, non per singolo pair: economico e
    # sufficiente, dato che il contesto generale cambia su scala di ore non di minuti.
    categorie_attive_ora = {categoria_di(p) for p in watchlist_attiva()}
    for categoria in categorie_attive_ora & CATEGORIE_STUDIO_GLOBALE:
        try:
            await aggiorna_contesto_mercato_categoria(bot, categoria)
        except Exception:
            logger.warning(f"Studio di mercato per '{categoria}' fallito in modo imprevisto, proseguo senza.")

    # --- FASE 1: analizza TUTTI i mercati e raccogli i segnali validi ---
    # Prima il bot apriva il PRIMO segnale valido incontrato scorrendo la lista: con
    # capitale piccolo questo significava esaurire il margine su un setup mediocre e
    # perdere quelli migliori piu' avanti nella lista. Ora raccoglie tutto e poi apre
    # in ordine di qualita' (04/09/2026).
    segnali_validi = []
    for pair in watchlist_attiva():
        ...
        if segnale:
            segnali_validi.append((score, pair, segnale))

        # Allerta depeg per le stablecoin - MAI un trade automatico, vedi la nota su
        # controlla_depeg_stablecoin(). Cooldown in memoria per non spammare Telegram
        # ad ogni ciclo di scansione finche' il depeg persiste.
        if categoria_di(pair) == "stablecoin":
            info_depeg = MONITORAGGIO.get(pair, {})
            if info_depeg.get("esito") == "depeg_rilevato":
                ultima_allerta = _ULTIMA_ALLERTA_DEPEG.get(pair)
                if not ultima_allerta or (ora - ultima_allerta) >= timedelta(hours=COOLDOWN_ALLERTA_DEPEG_ORE):
                    _ULTIMA_ALLERTA_DEPEG[pair] = ora
                    dev = info_depeg.get("deviazione_percento", 0)
                    prezzo = info_depeg.get("prezzo")
                    testo_allerta = (
                        f"🚨 *Possibile DEPEG rilevato*\n"
                        f"`{pair}` quota `{prezzo}` USD ({'+' if dev>0 else ''}{dev}% dal peg di 1.00)\n\n"
                        f"Questo e' solo un monitoraggio - il bot NON apre trade automatici su "
                        f"stablecoin. Verifica la situazione prima di decidere se e come intervenire "
                        f"manualmente: un depeg puo' essere un rimbalzo temporaneo o permanente."
                    )
                    try:
                        await bot.send_message(chat_id=CHANNEL_ID, text=testo_allerta, parse_mode=ParseMode.MARKDOWN)
                    except TelegramError as e:
                        logger.error(f"Errore invio allerta depeg per {pair}: {e}")
                    await notifica_admin(bot, testo_allerta)

        await asyncio.sleep(1)

    # --- FASE 2: apri partendo dal punteggio piu' alto ---
    segnali_validi.sort(key=lambda x: x[0], reverse=True)
    if segnali_validi:
        logger.info(
            "Segnali validi in questo ciclo (in ordine di punteggio): "
            + ", ".join(f"{p} {s}/100" for s, p, _ in segnali_validi)
        )

    for score, pair, segnale in segnali_validi:
        limite_posizioni = CONFIG.get("max_posizioni_aperte", 1)
        if len(posizioni_aperte) >= limite_posizioni:
            logger.info(
                f"Raggiunto il limite di {limite_posizioni} posizioni aperte contemporaneamente: "
                f"i segnali rimanenti di questo ciclo non vengono aperti."
            )
            break
...
# ---------------------------------------------------------------------------
# ACCESSO DA BROWSER (dashboard web fuori da Telegram)
# ---------------------------------------------------------------------------
# Fuori da Telegram non c'e' initData firmato, quindi si entra con WEB_PASSWORD.
# Il login restituisce un token casuale (sessione) che il browser manda in ogni
# richiesta nell'header "Authorization: Bearer <token>". Le sessioni vivono solo in
# memoria: a ogni riavvio/deploy del bot vanno rifatte (scelta voluta, piu' sicura).
# Protezioni: confronto a tempo costante, massimo 5 tentativi sbagliati ogni 15 minuti
# per indirizzo IP e 20 in totale (blocca anche chi prova da tanti IP diversi),
# notifica Telegram a Fabio a ogni accesso riuscito.

DURATA_SESSIONE_WEB_SECONDI = 7 * 24 * 3600
FINESTRA_TENTATIVI_SECONDI = 15 * 60
MAX_TENTATIVI_PER_IP = 5
MAX_TENTATIVI_TOTALI = 20
SESSIONI_WEB: dict[str, float] = {}           # token -> scadenza (timestamp)
TENTATIVI_FALLITI: dict[str, list[float]] = {}  # ip -> timestamp dei tentativi sbagliati


def accesso_web_abilitato() -> bool:
    return len(WEB_PASSWORD) >= LUNGHEZZA_MINIMA_WEB_PASSWORD


def _ip_richiesta(request) -> str:
    # Railway sta dietro un proxy: l'IP vero del visitatore e' il primo di X-Forwarded-For.
    inoltrato = request.headers.get("X-Forwarded-For", "")
    if inoltrato:
        return inoltrato.split(",")[0].strip()
    return request.remote or "sconosciuto"


def _pulisci_tentativi(adesso: float):
    for ip in list(TENTATIVI_FALLITI):
        TENTATIVI_FALLITI[ip] = [x for x in TENTATIVI_FALLITI[ip] if adesso - x < FINESTRA_TENTATIVI_SECONDI]
        if not TENTATIVI_FALLITI[ip]:
            del TENTATIVI_FALLITI[ip]
    for token in [k for k, scad in SESSIONI_WEB.items() if scad < adesso]:
        del SESSIONI_WEB[token]


def sessione_web_valida(request) -> bool:
    if not accesso_web_abilitato():
        return False
    intestazione = request.headers.get("Authorization", "")
    if not intestazione.startswith("Bearer "):
        return False
    token = intestazione[7:].strip()
    scadenza = SESSIONI_WEB.get(token)
    if not scadenza:
        return False
    if scadenza < time.time():
        SESSIONI_WEB.pop(token, None)
        return False
    return True


def richiesta_autorizzata(request, init_data: str) -> bool:
    """Vale sia per la Mini App dentro Telegram (initData firmato) sia per il
    browser (sessione ottenuta con WEB_PASSWORD)."""
    return bool(valida_init_data(init_data)) or sessione_web_valida(request)


async def handle_api_login(request):
    if not accesso_web_abilitato():
        return web.json_response(
            {"errore": "Accesso da browser non attivo: imposta WEB_PASSWORD (almeno 12 caratteri) su Railway"},
            status=403,
        )
    adesso = time.time()
    _pulisci_tentativi(adesso)
    ip = _ip_richiesta(request)
    totali = sum(len(v) for v in TENTATIVI_FALLITI.values())
    if len(TENTATIVI_FALLITI.get(ip, [])) >= MAX_TENTATIVI_PER_IP or totali >= MAX_TENTATIVI_TOTALI:
        return web.json_response({"errore": "Troppi tentativi sbagliati: riprova tra 15 minuti"}, status=429)
    ...
    # Confronto a tempo costante sugli hash (evita di rivelare la lunghezza o i caratteri giusti).
    ok = hmac.compare_digest(
        hashlib.sha256(password.encode("utf-8")).digest(),
        hashlib.sha256(WEB_PASSWORD.encode("utf-8")).digest(),
    )
    bot = request.app["stato_condiviso"].get("bot")
    if not ok:
        TENTATIVI_FALLITI.setdefault(ip, []).append(adesso)
        logger.warning(f"Login dashboard web fallito da {ip}")
        if bot and len(TENTATIVI_FALLITI[ip]) == MAX_TENTATIVI_PER_IP:
            await notifica_admin(bot, f"🚨 *Dashboard web*: {MAX_TENTATIVI_PER_IP} password sbagliate da IP `{ip}`. "
                                      f"Accesso bloccato per 15 minuti da quell'indirizzo.")
        return web.json_response({"errore": "Password errata"}, status=401)

    token = secrets.token_urlsafe(32)
    SESSIONI_WEB[token] = adesso + DURATA_SESSIONE_WEB_SECONDI
    TENTATIVI_FALLITI.pop(ip, None)
    logger.info(f"Login dashboard web riuscito da {ip}")
    if bot:
        await notifica_admin(bot, f"🔐 *Nuovo accesso alla dashboard web* da IP `{ip}`.\n"
                                  f"Se non sei stato tu, cambia subito WEB_PASSWORD su Railway: "
                                  f"al riavvio tutte le sessioni vengono chiuse.")
    return web.json_response({"ok": True, "token": token, "scadenza": SESSIONI_WEB[token]})


async def handle_api_logout(request):
    intestazione = request.headers.get("Authorization", "")
    if intestazione.startswith("Bearer "):
        SESSIONI_WEB.pop(intestazione[7:].strip(), None)
...
async def handle_api_data(request):
    ...
    if not richiesta_autorizzata(request, init_data):
        return web.json_response({"errore": "Accesso non autorizzato"}, status=401)
        ...
        try:
            ...
            for p in grezze:
                posizioni_reali.append({
                    ...
                    "leva": p.get("leverage"),
                    # Margine impegnato: serve alla dashboard per mostrare il rendimento in
                    # percentuale come lo calcola Bitget (ROE = P&L non realizzato / margine).
                    "margine": p.get("marginSize") or p.get("margin"),
            ...
            "strategie": [
                {
                    "chiave": chiave,
                    **info,
                    "statistiche": statistiche.get("per_strategia", {}).get(
                        chiave, {"vinti": 0, "persi": 0, "pareggi": 0, "pnl_usd": 0.0}),
                }
                for chiave, info in STRATEGIE.items()
            ],
            "suggerimento": suggerimento_strategia(statistiche),
            ...
            "categorie_disponibili": [
                {
                    ...
                    "soglie": soglie_di(cat["simboli"][0]) if cat["simboli"] else SOGLIE_PER_CATEGORIA_DEFAULT["crypto_major"],
                }
                for chiave, cat in CATEGORIE_WATCHLIST.items()
            ],
            "mercati_attivi": len(watchlist_attiva()),
            "score_minimo": CONFIG.get("score_minimo", SCORE_MINIMO_PUBBLICAZIONE_DEFAULT),
            "finestra_rsi_candele": CONFIG.get("finestra_rsi_candele", 5),
            "finestra_macd_candele": CONFIG.get("finestra_macd_candele", 3),
            "atr_moltiplicatore_sl": CONFIG.get("atr_moltiplicatore_sl", 2.0),
            "atr_moltiplicatore_tp1": CONFIG.get("atr_moltiplicatore_tp1", 1.2),
            "atr_moltiplicatore_tp2": CONFIG.get("atr_moltiplicatore_tp2", 2.5),
            "atr_moltiplicatore_tp3": CONFIG.get("atr_moltiplicatore_tp3", 4.5),
            "rispetta_orari_azioni": CONFIG.get("rispetta_orari_azioni", True),
            "intervallo_scansione_secondi": CONFIG.get("intervallo_scansione_secondi", SCAN_INTERVAL_SECONDS),
            "studio_mercato_globale_attivo": CONFIG.get("studio_mercato_globale_attivo", True),
            "studio_mercato_intervallo_ore": CONFIG.get("studio_mercato_intervallo_ore", 3),
            "escalation_soglia_dopo_perdite_attiva": CONFIG.get("escalation_soglia_dopo_perdite_attiva", False),
            "max_posizioni_aperte": CONFIG.get("max_posizioni_aperte", 1),
            "contesto_mercato_categorie": {
                cat: {
                    "bonus_soglia": dati_cat.get("bonus_soglia", 0),
                    "rischio_alto": dati_cat.get("rischio_alto", False),
                    "motivo": dati_cat.get("motivo", ""),
                    "aggiornato": dati_cat["timestamp"].isoformat() if dati_cat.get("timestamp") else None,
                }
                for cat, dati_cat in _CONTESTO_MERCATO_CATEGORIA.items()
            },
        },
        ...
        "posizioni_tracciate": [
            {
                ...
                "strategia": getattr(pos["segnale"], "strategia", "completo"),
                "simbolo_bitget": pair_kraken_a_bitget(pair),
            }
            for pair, pos in posizioni_aperte.items()
        ],
        "statistiche": {
...
_SIMBOLI_CONOSCIUTI = None


def simbolo_ammesso(pair: str) -> bool:
    """Il grafico accetta solo simboli presenti nelle categorie della watchlist, cosi'
    l'endpoint non diventa un proxy aperto verso qualunque simbolo Bitget."""
    global _SIMBOLI_CONOSCIUTI
    if _SIMBOLI_CONOSCIUTI is None:
        _SIMBOLI_CONOSCIUTI = {s for cat in CATEGORIE_WATCHLIST.values() for s in cat["simboli"]}
    return pair in _SIMBOLI_CONOSCIUTI


async def handle_api_candles(request):
    """Candele per il grafico stile MT5 della Mini App: OHLC, EMA50/EMA200 e i livelli
    della posizione aperta (se il bot ne sta gestendo una su quel simbolo)."""
    if not richiesta_autorizzata(request, request.query.get("initData", "")):
        return web.json_response({"errore": "Autenticazione Telegram non valida"}, status=401)
    pair = request.query.get("pair", "")
    if not simbolo_ammesso(pair):
        return web.json_response({"errore": "Simbolo non ammesso"}, status=400)
    try:
        tf = int(request.query.get("tf", "60"))
    except ValueError:
        tf = 60
    if tf not in (15, 60, 240):
        return web.json_response({"errore": "Timeframe non valido (15, 60, 240)"}, status=400)

    try:
        df = await asyncio.to_thread(get_ohlc, pair, tf, 300)
    except Exception as e:
        return web.json_response({"errore": f"Dati non disponibili: {e}"}, status=502)
    ...
    for riga in df.itertuples(index=False):
        candele.append({
            "t": int(pd.Timestamp(riga.time).timestamp()),
            "o": float(riga.open), "h": float(riga.high), "l": float(riga.low), "c": float(riga.close),
            "v": float(riga.volume),
            "e50": None if pd.isna(riga.ema50) else float(riga.ema50),
            "e200": None if pd.isna(riga.ema200) else float(riga.ema200),
        })

    livelli = None
    pos = request.app["stato_condiviso"]["posizioni_aperte"].get(pair)
    if pos:
        s = pos["segnale"]
        # Mostriamo solo i target che hanno davvero un ordine su Bitget: con capitale
        # piccolo il bot puo' aver piazzato 2 soli livelli (o uno), e disegnare linee
        # senza ordine dietro farebbe credere che li' debba succedere qualcosa.
        livelli = {
            "direzione": s.direzione.value, "entry": s.entry, "stop_loss": s.stop_loss,
            "tp1": s.tp1 if pos.get("quantita_tp1", 0) > 0 or not TRADING_ABILITATO else None,
            "tp2": s.tp2 if pos.get("quantita_tp2", 0) > 0 or not TRADING_ABILITATO else None,
            "tp3": s.take_profit,
            "strategia": getattr(s, "strategia", "completo"),
        }
    return web.json_response({
        "pair": pair, "simbolo": pair_kraken_a_bitget(pair), "tf": tf,
        "candele": candele, "livelli": livelli,
        "analisi": MONITORAGGIO.get(pair),
    })
...
async def handle_api_action(request):
    ...
    if not richiesta_autorizzata(request, body.get("initData", "")):
        return web.json_response({"errore": "Accesso non autorizzato"}, status=401)
    ...
    if azione == "pausa":
        ...
        nuova = body.get("valore")
        if nuova not in STRATEGIE:
            return web.json_response({"errore": "Strategia non valida"}, status=400)
    ...
    elif azione == "score_minimo":
        try:
            valore = int(body.get("valore"))
        except (TypeError, ValueError):
            return web.json_response({"errore": "Valore soglia score non valido"}, status=400)
        if not (0 <= valore <= 100):
            return web.json_response({"errore": "La soglia score deve essere tra 0 e 100"}, status=400)
        CONFIG["score_minimo"] = valore
        logger.info(f"Soglia score minima cambiata a {valore} dalla Mini App.")
    elif azione == "max_posizioni":
        try:
            valore = int(body.get("valore"))
        except (TypeError, ValueError):
            return web.json_response({"errore": "Numero di posizioni non valido"}, status=400)
        if not (1 <= valore <= 10):
            return web.json_response({"errore": "Le posizioni contemporanee devono essere tra 1 e 10"}, status=400)
        CONFIG["max_posizioni_aperte"] = valore
        logger.info(f"Massimo posizioni aperte contemporaneamente cambiato a {valore} dalla Mini App.")
    elif azione == "finestra_conferma":
        # body: {"campo": "rsi"|"macd", "valore": int}
        campo = body.get("campo")
        chiave_config = {"rsi": "finestra_rsi_candele", "macd": "finestra_macd_candele"}.get(campo)
        if not chiave_config:
            return web.json_response({"errore": "Campo finestra non valido (usa 'rsi' o 'macd')"}, status=400)
        try:
            valore = int(body.get("valore"))
        except (TypeError, ValueError):
            return web.json_response({"errore": "Valore finestra non valido"}, status=400)
        if not (1 <= valore <= 20):
            return web.json_response({"errore": "La finestra deve essere tra 1 e 20 candele"}, status=400)
        CONFIG[chiave_config] = valore
        logger.info(f"Finestra conferma '{campo}' cambiata a {valore} candele dalla Mini App.")
    elif azione == "atr_moltiplicatori":
        # body: {"sl": float, "tp1": float, "tp2": float, "tp3": float} - anche parziale
        mappa = {
            "sl": "atr_moltiplicatore_sl", "tp1": "atr_moltiplicatore_tp1",
            "tp2": "atr_moltiplicatore_tp2", "tp3": "atr_moltiplicatore_tp3",
        }
        nuovi_valori = {}
        for campo, chiave_config in mappa.items():
            if campo not in body:
                continue
            try:
                valore = float(body[campo])
            except (TypeError, ValueError):
                return web.json_response({"errore": f"Valore '{campo}' non valido"}, status=400)
            if not (0.1 <= valore <= 15):
                return web.json_response({"errore": f"'{campo}' deve essere tra 0.1 e 15 (multipli di ATR)"}, status=400)
            nuovi_valori[chiave_config] = valore
        if not nuovi_valori:
            return web.json_response({"errore": "Nessun moltiplicatore fornito (usa sl/tp1/tp2/tp3)"}, status=400)
        # Coerenza minima: TP1 < TP2 < TP3, cosi' i tre target parziali restano ordinati.
        finale = {
            "sl": nuovi_valori.get("atr_moltiplicatore_sl", CONFIG["atr_moltiplicatore_sl"]),
            "tp1": nuovi_valori.get("atr_moltiplicatore_tp1", CONFIG["atr_moltiplicatore_tp1"]),
            "tp2": nuovi_valori.get("atr_moltiplicatore_tp2", CONFIG["atr_moltiplicatore_tp2"]),
            "tp3": nuovi_valori.get("atr_moltiplicatore_tp3", CONFIG["atr_moltiplicatore_tp3"]),
        }
        if not (finale["tp1"] < finale["tp2"] < finale["tp3"]):
            return web.json_response({"errore": "I target devono crescere: TP1 < TP2 < TP3"}, status=400)
        CONFIG.update(nuovi_valori)
        logger.info(f"Moltiplicatori ATR aggiornati dalla Mini App: {nuovi_valori}")
    elif azione == "soglie_categoria":
        # body: {"categoria": "azioni", "atr_min": float, "ema_min": float} - anche parziale
        chiave_categoria = body.get("categoria")
        if chiave_categoria not in CATEGORIE_WATCHLIST:
            return web.json_response({"errore": f"Categoria sconosciuta: {chiave_categoria}"}, status=400)
        aggiornamento = {}
        for campo in ("atr_min", "ema_min"):
            if campo not in body:
                continue
            try:
                valore = float(body[campo])
                ...
                return web.json_response({"errore": f"Valore '{campo}' non valido"}, status=400)
            if not (0.0 <= valore <= 20):
                return web.json_response({"errore": f"'{campo}' deve essere tra 0 e 20"}, status=400)
            aggiornamento[campo] = valore
        if not aggiornamento:
            return web.json_response({"errore": "Nessuna soglia fornita (usa atr_min/ema_min)"}, status=400)
        soglie_categorie = CONFIG.setdefault("soglie_categorie", {})
        soglie_categorie.setdefault(chiave_categoria, {}).update(aggiornamento)
        logger.info(f"Soglie categoria '{chiave_categoria}' aggiornate dalla Mini App: {aggiornamento}")
    elif azione == "orari_azioni":
        nuovo = body.get("valore")
        if not isinstance(nuovo, bool):
            return web.json_response({"errore": "Valore non valido (atteso true/false)"}, status=400)
        CONFIG["rispetta_orari_azioni"] = nuovo
        logger.info(f"Rispetto orari di borsa per azioni/ETF: {'attivo' if nuovo else 'disattivato'} (dalla Mini App)")
    elif azione == "intervallo_scansione":
        try:
            valore = int(body.get("valore"))
        except (TypeError, ValueError):
            return web.json_response({"errore": "Valore intervallo non valido"}, status=400)
        if not (60 <= valore <= 1800):
            return web.json_response({"errore": "L'intervallo di scansione deve essere tra 60 e 1800 secondi"}, status=400)
        CONFIG["intervallo_scansione_secondi"] = valore
        logger.info(f"Intervallo di scansione cambiato a {valore}s dalla Mini App.")
    elif azione == "studio_mercato":
        nuovo = body.get("valore")
        if not isinstance(nuovo, bool):
            return web.json_response({"errore": "Valore non valido (atteso true/false)"}, status=400)
        CONFIG["studio_mercato_globale_attivo"] = nuovo
        logger.info(f"Studio generale di mercato per categoria: {'attivo' if nuovo else 'disattivato'} (dalla Mini App)")
    elif azione == "studio_mercato_intervallo":
        try:
            valore = int(body.get("valore"))
        except (TypeError, ValueError):
            return web.json_response({"errore": "Valore intervallo non valido"}, status=400)
        if not (1 <= valore <= 24):
            return web.json_response({"errore": "L'intervallo deve essere tra 1 e 24 ore"}, status=400)
        CONFIG["studio_mercato_intervallo_ore"] = valore
        logger.info(f"Intervallo studio di mercato cambiato a {valore}h dalla Mini App.")
    elif azione == "escalation_perdite":
        nuovo = body.get("valore")
        if not isinstance(nuovo, bool):
            return web.json_response({"errore": "Valore non valido (atteso true/false)"}, status=400)
        CONFIG["escalation_soglia_dopo_perdite_attiva"] = nuovo
        logger.info(f"Escalation soglia dopo perdite consecutive: {'attiva' if nuovo else 'disattivata'} (dalla Mini App)")
    else:
...
async def tieni_sveglio():
    """Render gratuito spegne il servizio dopo 15 minuti senza visite dall'esterno:
    ogni 10 minuti il bot visita il proprio indirizzo pubblico, cosi' resta acceso.
    Su Railway o su un VPS (niente RENDER_EXTERNAL_URL) non fa nulla."""
    if not RENDER_EXTERNAL_URL:
        return
    logger.info(f"Keep-alive attivo: visito {RENDER_EXTERNAL_URL}/ping ogni 10 minuti.")
    while True:
        await asyncio.sleep(600)
        try:
            await asyncio.to_thread(requests.get, f"{RENDER_EXTERNAL_URL}/ping", timeout=20)
        except Exception as e:
            logger.warning(f"Keep-alive non riuscito: {e}")
...
async def avvia_web_server(stato_condiviso: dict):
    ...
    app.router.add_get("/api/candles", handle_api_candles)
    ...
    if accesso_web_abilitato():
        logger.info("Accesso da browser ATTIVO (WEB_PASSWORD impostata).")
    elif WEB_PASSWORD:
        logger.warning(f"WEB_PASSWORD troppo corta (minimo {LUNGHEZZA_MINIMA_WEB_PASSWORD} caratteri): accesso da browser SPENTO.")
    else:
        logger.info("Accesso da browser spento (WEB_PASSWORD non impostata): dashboard solo dentro Telegram.")
...
async def cmd_start(update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/strategia — Expert Advisor attivo, suggerimento in base al mercato, cambio strategia\n"
...
async def cmd_help(update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/monitoraggio — score, condizioni, volatilita' e regime (ADX) per ogni coppia, aggiornati all'ultima scansione\n"
        "/strategia — quale strategia e' attiva, quale conviene col mercato attuale, e pulsanti per cambiarla\n"
...
async def cmd_dashboard(update, context: ContextTypes.DEFAULT_TYPE):
    if not DOMINIO_PUBBLICO:
        ...
        return
    url = f"https://{DOMINIO_PUBBLICO}/"
    tastiera = InlineKeyboardMarkup([[InlineKeyboardButton("📊 Apri Dashboard", web_app=WebAppInfo(url=url))]])
    if accesso_web_abilitato():
        nota_web = f"\n\n🌐 Da browser (PC, tablet): {url}\nServe la password WEB_PASSWORD impostata su Railway."
    else:
        nota_web = ("\n\n🌐 Per aprirla anche da browser, imposta su Railway la variabile "
                    f"WEB_PASSWORD (almeno {LUNGHEZZA_MINIMA_WEB_PASSWORD} caratteri).")
    await update.message.reply_text("Tocca per aprire la dashboard:" + nota_web, reply_markup=tastiera)
...
def _utente_autorizzato(update) -> bool:
    """Chi puo' CAMBIARE la strategia da Telegram: l'utente della Mini App
    (AUTHORIZED_USER_ID) o la chat admin. Se nessuno dei due e' configurato, nessuno."""
    utente = getattr(update, "effective_user", None)
    chat = getattr(update, "effective_chat", None)
    if AUTHORIZED_USER_ID and utente and str(utente.id) == str(AUTHORIZED_USER_ID):
        return True
    if ADMIN_CHAT_ID and chat and str(chat.id) == str(ADMIN_CHAT_ID):
        return True
    return False


def _testo_strategia(statistiche: dict) -> str:
    attiva = CONFIG.get("modalita_segnale", "completo")
    sugg = suggerimento_strategia(statistiche)
    righe = [f"🤖 *Expert Advisor attivo:* {STRATEGIE[attiva]['nome']}",
             f"_{STRATEGIE[attiva]['descrizione']}_", ""]
    if sugg.get("strategia_consigliata"):
        consigliata = STRATEGIE[sugg["strategia_consigliata"]]["nome"]
        righe.append(f"🧭 *Suggerita ora:* {consigliata}")
        if sugg.get("alternativa"):
            righe.append(f"   alternativa: {STRATEGIE[sugg['alternativa']]['nome']}")
        righe.append(f"_{sugg['motivo']}_")
        if sugg.get("cautela"):
            righe.append(f"⚠️ {sugg['cautela']}")
        if sugg.get("adx_mediano") is not None:
            righe.append(f"ADX mediano 4h: {sugg['adx_mediano']} su {sugg['mercati_analizzati']} mercati")
    else:
        righe.append(f"🧭 {sugg['motivo']}")
    migliore = sugg.get("migliore_sui_tuoi_dati")
    righe.append("")
    if migliore:
        righe.append(f"📈 *Sui tuoi dati reali* ({migliore['trade']} trade): la migliore finora e' "
                     f"{STRATEGIE.get(migliore['strategia'], {}).get('nome', migliore['strategia'])} "
                     f"({'+' if migliore['pnl_usd'] >= 0 else ''}{migliore['pnl_usd']} USDT)")
    else:
        righe.append(f"📈 Nessuna strategia ha ancora {MIN_TRADE_PER_STATISTICA_AFFIDABILE} trade chiusi: "
                     "il suggerimento e' teorico, non verificato sul tuo conto.")
    righe.append("\n_Nessuna strategia garantisce profitti. Il suggerimento indica la condizione di "
                 "mercato, non l'esito dei trade._")
    return "\n".join(righe)


def _tastiera_strategie() -> InlineKeyboardMarkup:
    attiva = CONFIG.get("modalita_segnale", "completo")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(("✅ " if chiave == attiva else "") + info["nome"],
                              callback_data=f"strategia:{chiave}")]
        for chiave, info in STRATEGIE.items()
    ])


async def cmd_strategia(update, context: ContextTypes.DEFAULT_TYPE):
    statistiche = context.bot_data.get("statistiche", {})
    tastiera = _tastiera_strategie() if _utente_autorizzato(update) else None
    await update.message.reply_text(_testo_strategia(statistiche), parse_mode=ParseMode.MARKDOWN,
                                    reply_markup=tastiera)


async def callback_strategia(update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not _utente_autorizzato(update):
        await query.answer("Non autorizzato a cambiare la strategia.", show_alert=True)
        return
    chiave = (query.data or "").split(":", 1)[-1]
    if chiave not in STRATEGIE:
        await query.answer("Strategia sconosciuta.")
        return
    CONFIG["modalita_segnale"] = chiave
    logger.info(f"Strategia cambiata a '{chiave}' da Telegram.")
    await query.answer(f"Attivata: {STRATEGIE[chiave]['nome']}")
    try:
        await query.edit_message_text(_testo_strategia(context.bot_data.get("statistiche", {})),
                                      parse_mode=ParseMode.MARKDOWN, reply_markup=_tastiera_strategie())
    except TelegramError as e:
        logger.info(f"Messaggio /strategia non aggiornato: {e}")
...
async def cmd_monitoraggio(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for pair in watchlist_attiva():
        ...
        if d["score"] < 0:
            ...
            continue
        atr_s = f"{d['atr_percento']}%" if d.get("atr_percento") is not None else "n/d"
        esito_s = ESITO_LEGGIBILE.get(d["esito"], d["esito"])
        if d.get("condizioni"):
            flag = "  ".join(f"{c['nome']} {'✓' if c['ok'] else '✗'}" for c in d["condizioni"])
        else:
            rsi_s = "—" if d.get("rsi_ok") is None else ("✓" if d["rsi_ok"] else "✗")
            macd_s = "—" if d.get("macd_ok") is None else ("✓" if d["macd_ok"] else "✗")
            vol_s = "—" if d.get("volume_ok") is None else ("✓" if d["volume_ok"] else "✗")
            flag = f"RSI {rsi_s}  MACD {macd_s}  Vol {vol_s}"
        regime_s = f"  ADX {d['adx']} ({d['regime']})" if d.get("adx") is not None else ""
        righe.append(
            f"`{pair}` {d.get('direzione', '')} — score *{d['score']}/{d.get('soglia_attuale', '–')}*{eta}\n"
            f"  {flag}  ATR {atr_s}{regime_s}  → {esito_s}"
        )
    # Telegram rifiuta messaggi oltre 4096 caratteri: con molte categorie attive la lista
    # li supera, quindi la spezziamo in piu' messaggi senza tagliare a meta' una riga.
    blocco = ""
    for riga in righe:
        if len(blocco) + len(riga) + 1 > 3800:
            await update.message.reply_text(blocco, parse_mode=ParseMode.MARKDOWN)
            blocco = ""
        blocco += riga + "\n"
    if blocco.strip():
        await update.message.reply_text(blocco, parse_mode=ParseMode.MARKDOWN)
...
async def main():
    ...
    application.add_handler(CommandHandler("strategia", cmd_strategia))
    application.add_handler(CallbackQueryHandler(callback_strategia, pattern=r"^strategia:"))
    bot = application.bot

    if backup_telegram_attivo():
        try:
            await bot.initialize()
            await ripristina_stato_da_telegram(bot)
        except Exception as e:
            logger.warning(f"Impossibile contattare Telegram per il ripristino dello stato: {e}")
    ...
    asyncio.create_task(tieni_sveglio())
    ...
    logger.info("Bot avviato. Scansione ogni %d secondi (modificabile dalla dashboard).", CONFIG.get("intervallo_scansione_secondi", SCAN_INTERVAL_SECONDS))
    while True:
        try:
        ...
        try:
            ...
            await backup_stato_su_telegram(bot)
        except Exception:
            tb = traceback.format_exc()
        ...
        await asyncio.sleep(CONFIG.get("intervallo_scansione_secondi", SCAN_INTERVAL_SECONDS))
