...
SOGLIA_DISTANZA_EMA_MINIMA = 0.5  # sotto questa distanza % tra EMA50 e EMA200 su 4h, il
# trend e' considerato troppo debole/appena nato per fidarsene (incrocio ancora incerto).
...
def analizza_coppia(pair: str, statistiche: dict = None) -> tuple[Segnale | None, int, str]:
    ...
    trend_abbastanza_forte = distanza_ema_percento >= SOGLIA_DISTANZA_EMA_MINIMA
    ...
    if bias == Direzione.LONG:
        rsi_ok = bool((finestra_rsi < 35).any() and ultimo["rsi"] >= 35)
    else:
        rsi_ok = bool((finestra_rsi > 65).any() and ultimo["rsi"] <= 65)
